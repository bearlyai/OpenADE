...
- Classic desktop Search tray content search and first-result preview now keep the old tray UI while routing repo-root/head-mode task searches through `CodeStore.searchProductProject()` and `readProductProjectFile()` when runtime product reads are active. Worktree task searches still use trusted-local `filesApi` until OpenADE exposes a task-scoped file/search API; routing those through repo-root project search would be a behavior regression.
...
- `ContentSearchManager` keeps the classic desktop Search tray behavior but uses `searchProductProject()` and `readProductProjectFile()` for runtime-backed repo-root/head-mode tasks. The manager deliberately falls back to trusted-local `filesApi` for worktree task directories until OpenADE owns a task-scoped file/search API.
