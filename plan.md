...
During the active migration, do not block local dogfooding on the full production rollout gate. For slices that are meant to make the app testable quickly, the local feedback gate is:

- touched package typecheck plus the repo no-`any` scanner
- focused real runtime/client coverage for the changed capability
- one focused shared-shell render/integration test when UI wiring changes
- a local launch or manual smoke note only when automated coverage cannot exercise the path cheaply

Do not require React Doctor, the full production-data import suite, packaged Electron restart smoke, mobile build/simulator pass, browser attach smoke, performance budget suite, or rollback drill for every local feedback slice. Run those when the slice affects React state/effects broadly, makes Core the default backend, removes fallback behavior, changes storage/import semantics, grants raw host/admin/migration authority, or flips a default. Reviewed paired-safe product grants only need focused capability, denial, and real remote-runtime coverage. Local feedback builds may keep explicit legacy fallback paths while users validate Core-backed behavior, but the final production exit criteria still require those fallbacks to be unused in normal product flow.

For local feedback, prefer a real but narrow proof over broad ceremony:

- one real Core/runtime permission test for server-side grants and denials
- one real `RuntimeClient`/`OpenADEClient` test for capability discovery and shell calls
- one shared-shell test that proves controls are visible/hidden from capabilities
- `typecheck` and the no-`any` scanner for touched TypeScript/Go packages

Do not add new migration gates just because a feature moved behind the shared shell. Add a production gate only when the change expands host authority, changes durable data, changes fallback policy, or flips a default.

## Capabilities And Shell First

Before more backend ownership migration, finish a dogfood-ready shared shell over the existing Core/runtime capability set.
...
- Task shell: lightweight task detail, turns, reviews, comments, queued-turn enqueue/reorder/cancel, metadata, plain delete, image reads, snapshot patch reads, resource inventory, capability-gated terminal, and git changes/diff/file-pair/log/scopes/summary/commit-file inspection.
...
The current local-feedback target is shared-shell parity for paired-safe product capabilities that can be tested without changing backend ownership defaults:

- project tasks, files, search, process list/output with capability-gated controls, git info/branches/summary, and read-only cron definitions
- task detail, turns, reviews, comments, queued-turn enqueue/reorder/cancel, metadata, delete, images, snapshot patches, resource inventory, capability-gated task terminal, and task git changes/diff/file-pair/log/scopes/summary/commit-file inspection
- remote session pairing, session switching, theme matching, device self-revoke, and runtime notification refresh behavior

Keep task opening and project opening on lightweight DTO reads. Capability panels should stay lazy/manual unless the classic desktop route already needs that data to render the first view.

2026-06-10 status: the paired-safe capabilities plus shared-shell gate is ready for local feedback. The shared shell derives visible controls from runtime capabilities, hides/short-circuits denied controls, and keeps task/project opening on lightweight DTO paths. Go Core, Electron companion, and the generated OpenADE client now have real paired-device coverage for snapshot/task reads, task create, turn start/interrupt, read-only cron definitions, queued-turn enqueue/reorder/cancel, self-revoke, and denial of raw/trusted powers such as repo creation, pairing admin, process start/stop, terminals, git commit, and file writes. Shared-shell title generation, task environment prepare, task git commit, and project file write are wired behind `openade/task/title/generate`, `openade/task/environment/prepare`, `openade/task/git/commit`, and `openade/project/file/write` capabilities with focused real-runtime coverage, but they remain hidden for paired clients unless those scoped methods are explicitly granted. Future local-feedback slices should add only focused capability, denial, real runtime/client, and touched-shell tests unless they expand host authority, change durable data/fallback semantics, or flip defaults.

Defer broader mutating/admin parity until after local feedback unless it blocks dogfooding: repo create/delete, MCP settings, cron install-state/run controls, personal settings writes, and any decision to grant title generation, environment prepare, git commit, or file writes to paired devices by default.
...
- 2026-06-11: Remote/shared project Files now exposes capability-gated file-name search through `openade/project/files/fuzzySearch` independently from the full file-tree load. `ProjectFilesPanel` can show a Find file control when only fuzzy search is granted, can read a selected fuzzy result through `openade/project/file/read`, and still keeps `openade/project/files/tree` behind the explicit Load Files action. The real `RemoteApp` runtime integration test proves project open performs zero file-tree and zero fuzzy-search reads, fuzzy search issues one scoped fuzzy-search request, reading its result does not load the tree, and the tree still loads only after the explicit Load Files action.

- 2026-06-11: Remote/shared existing-task input now uses `openade/queued-turn/enqueue` when the selected task is already running and the command is queueable Do/Ask. `OpenADEShell` and `TaskScreen` distinguish `openade/turn/start` from queued-turn enqueue capability so the composer only enables the operation actually granted by the runtime. The real `RemoteApp` runtime integration test simulates a running task over the in-memory runtime transport and proves the shell issues one queued-turn enqueue request, zero turn-start requests, and no snapshot/task rereads for its own accepted queue mutation.

- 2026-06-11: Remote/shared New Task now uses `openade/task/create` as the task-record mutation instead of relying on `openade/turn/start` to implicitly create the task. When `openade/turn/start` is also advertised, the shell labels the action Create & Run and starts a turn in the newly created task; when only `openade/task/create` is advertised, it creates the task without exposing agent/mode controls or starting execution. The product store now exposes `createTask()` and patches project/task caches from the create result, while the real `RemoteApp` runtime integration test proves both create-and-run and create-only capability profiles without snapshot/task rereads.

- 2026-06-11: Remote/shared project navigation no longer eagerly calls `openade/project/files/tree`. `ProjectFilesPanel` now remains visible in an unloaded state when the runtime grants file-tree access, and `RemoteApp` loads project task previews on project entry while leaving file-tree reads behind the explicit Load Files action. The real `RemoteApp` runtime integration test now proves project open performs zero file-tree reads, then proves the first Files panel load issues one scoped tree read before file read/write/search flows continue through `OpenADEProductStore`.

- 2026-06-10: Remote/shared task snapshot patch viewing now prefers indexed slice browsing when the runtime advertises `openade/task/snapshot/index/read` and `openade/task/snapshot/patch/readSlice`, falling back to the older full `openade/task/snapshot/patch/read` for compatibility. `TaskEventThread` renders the Core-built patch index, loads a selected file slice on demand, and keeps the snapshot block lazy so task opening stays on lightweight task DTO reads. The real `RemoteApp` runtime integration test proves the shared shell issues an index read, does not call full patch read, and then fetches only the selected file slice.

- 2026-06-10: Electron companion paired-device permissions now align with Go Core's paired profile for repo management. Paired clients no longer receive `openade/repo/create`, `openade/repo/update`, or `openade/repo/delete` by default because repo path management is a host-authority decision that needs an explicit role/permission model before mobile/browser clients can safely surface it. The companion WebSocket integration now proves those methods are hidden from paired capabilities and denied over the real authenticated runtime socket while normal paired task/comment/queued-turn/task-delete mutations remain available.

- 2026-06-10: Electron companion paired-device permissions now also align with Go Core by removing generic `agent/provider/list`, `agent/provider/status`, `agent/serverProtocol/list`, `agent/approval/*`, and `agent/approval/requested` notification grants from paired devices. Agent provider state, server-protocol session details, and approval controls remain trusted/local runtime capabilities until they have an explicit OpenADE product role, Core parity, and real paired WebSocket coverage. The companion capability test now proves these methods/notifications stay hidden and direct paired requests are permission-denied.

- 2026-06-10: `openade/task/create` is now a first-class route in the TypeScript OpenADE module as well as Go Core. The compatibility module routes it through the existing Yjs writer, Node and Electron hosts publish task/preview/snapshot invalidations for created tasks, Electron paired-device permissions now include it to match Core's paired profile, and real module plus paired companion WebSocket tests cover creating, reading, and deleting a task without starting execution.

- 2026-06-10: Electron companion paired notification permissions now match Go Core's paired profile: `connection/*`, `remote/device/changed`, and `openade/*`. This removes the earlier broad `remote/*` grant so future remote/admin notifications cannot leak to paired devices by prefix accident, while keeping Core-compatible connection lag/recovery notifications available. The companion WebSocket notification filter test registers a fake `remote/debug` notification and proves it is hidden while `remote/device/changed`, `connection/*`, and `openade/*` notifications are delivered.

- 2026-06-10: Go Core slow-request telemetry now covers WebSocket protocol decode failures before handler dispatch. Malformed JSON and invalid request-id errors are timestamped after the text frame is received, wait in the same sequential request queue as normal calls, and emit `SlowRequestEvent` records when the total queue+decode time crosses the threshold. If no valid method is available, the event uses the synthetic method `protocol/decode`; it still reports sanitized service, request id, connection id, queue wait, handler/decode time, failed state, and `invalid_request` error code without logging raw request payloads. A real WebSocket regression sends malformed JSON behind a slow handler and proves the resulting protocol-error slow event includes queue wait behind the slow handler.

- 2026-06-10: `CodeStore` now emits `runtime_product_store_fallback` telemetry for legacy `localOpenADEClient` product-method fallbacks, not only direct legacy task-store reads. Every selected product API wrapper that drops to the legacy local client records `source: "legacy_product_client"` with the OpenADE method name as `reason`, deduped through the existing fallback key path and enriched with Core rollout state, runtime product status, snapshot/task counts, and transport. This gives rollout review a concrete signal when normal product commands still escape Core after runtime-product migration is enabled. A focused store regression proves an enabled runtime-product session that falls back to legacy repo creation emits the fallback event before the legacy IPC request fails. Remote guidance now treats Yjs-backed projections as legacy compatibility and directs new remote product behavior through `OpenADEProductStore` and the selected OpenADE runtime/Core API.

- 2026-06-10: Clean managed-Core desktop startup now fails closed on Core snapshot errors instead of silently falling back to legacy product storage. When rollout says Core is connected, non-legacy, and no legacy Yjs documents are present, `CodeStore` uses product personal settings, an ephemeral MCP projection, Core-owned product APIs, Core-owned runtime hydration, and Core-owned cron scheduling even if `openade/snapshot/read` fails. It keeps the runtime product store in an error/no-snapshot state with no repos rather than connecting legacy repo, MCP, personal-settings, runtime-list, or cron stores, and product helpers, manager-level repo CRUD, notification refreshes, image writes, generic task reads, task-model git reads, snapshot artifact reads, task/comment/repeat/input mutations, MCP settings, and cron product state keep targeting the selected Core product API instead of legacy `localOpenADEClient`, raw Electron helpers, or Yjs. Manager/page/model code should use `CodeStore.shouldUseRuntimeProductAPI()` when choosing an API boundary and `shouldUseRuntimeProductReads()` only when a populated snapshot projection is actually required. Directly loaded runtime task DTOs remain valid even when the snapshot projection is null, so accepted task mutations sync back into `runtimeProductTasks` by using the cached task's repo id before consulting the snapshot. The real runtime-backed store regression forces snapshot refresh to fail while product settings still work, proves all legacy connectors remain unopened, proves active runtime state hydrates from the selected Core client, proves renderer cron stays off even if `CronManager.startAll()` is called manually, loads a task directly through Core while the snapshot remains null, refreshes task git state and snapshot patch content through Core from that cached task, mutates its comments without follow-up task/snapshot reads, then creates a repo through `RepoManager` and the selected Core client before a Core snapshot refresh recovers the UI cache.

- 2026-06-10: Go Core WebSocket request timing now starts after a text message is received, not before the blocking `socket.Read()` call, and received messages are now queued before sequential handler execution. This prevents idle connected clients from inflating `queueWaitMs`, while still making requests that are already waiting behind a slow handler report that head-of-line delay as runtime queue wait. Real WebSocket regressions initialize a client, prove idle socket time does not emit a slow event for a later fast method, then send a slow and fast request back-to-back and prove the fast request's slow-event records the wait behind the slow handler as queue time.

- 2026-06-10: Electron Yjs storage now keeps the short load cache warm after an unchanged save. `handleSave()` still invalidates before merging writes, but when the incoming update exactly matches the persisted bytes it re-caches those bytes before returning, so renderer debounced no-op saves no longer evict the cache and force the next `openade/snapshot/read`, `openade/task/read`, or storage refresh to reload/decode the same document from disk. The real filesystem storage regression saves, loads, performs an unchanged save, and proves the following load does not log a disk-backed `Loaded document`.

- 2026-06-10: Runtime-backed `CodeStore.refreshRepoStoreFromStorage()` and `refreshTaskStoreFromStorage()` now short-circuit to Core/runtime DTO refreshes before touching any legacy Yjs connection. This keeps old helper names usable for legacy callers while preventing runtime-active notification and mutation paths from reopening unchanged `code:repos`, `code:personal_settings`, or multi-megabyte task documents just because a legacy connection still exists in memory. The desktop bridge regression installs legacy repo/task refresh connections alongside a real `RuntimeServer`/`OpenADEClient` product store and proves the explicit refresh helpers issue runtime reads without calling legacy Yjs refresh.

- 2026-06-10: Runtime-backed `TaskModel` instances no longer run the legacy after-event git-state refresh when an execution settles. The runtime DTO settlement path still fires `CodeStore.execution.onAfterEvent()` callbacks for analytics/repeat/notification behavior, but it does not trigger an implicit `openade/task/git/summary/read`; task git state remains explicit user work through the Changes/Git surfaces. The real `CodeStore` runtime bridge settlement regression now counts task git-summary reads and proves runtime completion updates task events plus after-event callbacks without hidden git work.

- 2026-06-10: Runtime-backed classic desktop task pages no longer install the legacy 20s git-status polling interval when a task is running. Core/runtime sessions keep task opening and running-state updates on task/runtime DTOs; task git status loads from explicit Changes/Git user actions instead of route-owned background polling. The real local-runtime classic route smoke now emits a running `runtime/updated` notification for the open task and proves no 20s polling interval is scheduled and no task git-summary read occurs.

- 2026-06-10: `OpenADEProductStore.handleNotification()` now suppresses matching self-accepted mutation and action-start notification echoes before running normal task/snapshot refresh logic. This makes the direct `CodeStore` runtime-notification bridge behave like the product store's own subscription wrapper: accepted metadata/comment/queued-turn echoes with the same `clientRequestId` do not reread task detail or snapshot, accepted in-progress action-start echoes with the same task/event do not reread task detail, and later external/terminal updates still refresh normally. Focused product-store regressions cover direct `openade/task/updated` plus `openade/task/previewChanged` echoes from accepted metadata writes and direct action-start echoes followed by a later completed update.

- 2026-06-10: Runtime-backed classic desktop task opening now defers `lastViewedAt` persistence instead of sending `openade/task/metadata/update` immediately from the route effect. `TaskManager.markTaskViewed(taskId, { defer: true })` patches the runtime product DTO cache synchronously so unread/sidebar UI updates, then coalesces the server metadata write behind a short timer; direct `markTaskViewed()` calls remain immediate for non-route callers. The real local-runtime desktop route smoke now proves task open does not emit an immediate viewed metadata update, and focused TaskManager timer tests prove deferred persistence fires once and is cancelled on store teardown.

- 2026-06-09: Runtime-backed classic desktop task opening no longer eagerly loads git/environment/SDK/file-search side effects. `TaskPage` keeps the route on the lightweight task DTO path, skips the delayed task git-summary refresh, skips the project git-info-backed environment load, and skips focus-triggered git refreshes while runtime product reads are active. `TaskModel.taskWorkingDirHint` gives editor/tray surfaces a repo/task-scoped directory identity without fetching git info, tray open handlers initialize Files/Search from that hint on demand, and `SmartEditor` now probes SDK slash-command capabilities only after the slash-command suggestion opens. The real local-runtime desktop route smoke proves task open renders the classic UI with zero task git-summary, project git-info, task changes, fuzzy file-search, or SDK-capability requests; it then performs an explicit product file-mention search to prove scoped file search still works on demand.

- 2026-06-09: Remote/shared task opening no longer eagerly calls task git reads. `RemoteApp` now clears previous git panel state on task switch and leaves `openade/task/changes/read` plus `openade/task/git/log` lazy behind the shared task git panel's explicit load/refresh button. The real `RemoteApp` + `RuntimeServer` + `OpenADEClient` integration test proves task detail renders with zero task changes/log runtime reads, then proves the first git-panel load makes one changes read and one git-log read before diff viewing calls the scoped diff route.

- 2026-06-09: Remote/shared project navigation no longer eagerly calls `openade/project/process/list` just because a project screen opened. `RemoteApp` now loads project files on project entry and leaves process definitions/instances lazy behind the shared process panel's explicit load/refresh command or process actions. The real `RemoteApp` + `RuntimeServer` + `OpenADEClient` integration test proves project files and search render with zero process-list runtime reads, then proves the first process-panel load is the only process-list read and accepted start/stop/output actions do not add duplicate process-list reads.

- 2026-06-09: Cron/sidebar config hydration now has a cron-only product read. Core, the TypeScript OpenADE module, OpenADEClient contracts, `OpenADEProductStore`, and `CodeStore` expose `openade/cron/definitions/read`, which returns parsed cron config groups and parse errors without process definitions or live process instances. `CronManager` uses that method for runtime-backed config refresh while Processes tray/editor flows keep using `openade/project/process/list` for the full process shape. Real Core WebSocket, product-store, CronManager, and runtime-backed desktop tests cover the route, cache reuse/invalidation, and the guarantee that cron-only hydration no longer calls broad process-list.

- 2026-06-09: The remote/shared new-task screen now uses the shared `TaskComposer` for command selection, prompt entry, submit-state handling, and agent controls instead of carrying a separate new-task command row and prompt/send button implementation. `TaskComposer` now accepts a scoped command list plus send gating/label/placeholder overrides, so existing task threads keep the full command set while new-task creation stays limited to Do/Plan/Ask/HyperPlan. Remote existing-task turns, reviews, and new-task creation now share one selected harness/model/thinking/fast-mode state and carry those values into the product API instead of hard-coding default agent settings. Classic desktop review starts now also preserve the current `TaskModel` thinking/fast-mode settings when launching review through `ReviewPickerModal`. `openade/review/start` now accepts optional thinking/fast-mode in both the TypeScript runtime module and Go Core, and the read-only review plus follow-up Ask executor requests inherit those settings. The TypeScript OpenADE runtime module also tags action-start `openade/task/updated` notifications with `eventId` and `eventStatus: "in_progress"` like Core and avoids separate `previewChanged` notifications for action starts, letting remote cancel self-start task refreshes without hiding later terminal updates. Core and the TypeScript module now tag accepted metadata, comment, and queued-turn mutation notifications with the sanitized `clientRequestId`, so remote clients and the shared `OpenADEProductStore` can suppress their own echoes without suppressing unrelated task updates. Remote review starts and accepted task mutations now sync from the accepted product-store cache instead of immediately rereading the selected task after acceptance. Accepted project process start/stop now also patch a fresh cached process list locally, so remote process controls do not immediately reread `openade/project/process/list` just to observe their own accepted start/stop. Focused shell tests cover the reused create controls/offline disable behavior, real `RuntimeServer`/`OpenADEClient` remote integration tests prove the shared composer values reach existing-task, new-task, and review runtime requests while accepted existing-task starts, new-task creation, metadata/comment/queued-turn mutations, process start/stop, and review starts do not issue redundant task/snapshot/process-list reads, a shared product-store regression proves tagged accepted mutation echoes do not reread task/snapshot state while untagged external updates still refresh, a classic desktop modal regression proves review thinking/fast-mode reaches the runtime product API without broad refreshes, and the Core review-start test proves review/follow-up execution inherits thinking/fast-mode.

- 2026-06-09: Runtime-backed stats usage backfill no longer performs a broad post-metadata refresh on non-clean runtime sessions. The fallback path still computes usage from one bounded runtime task read when Core bulk backfill is unavailable, then relies on the accepted metadata update to patch cached task-preview usage locally. A real `RuntimeServer`/`OpenADEClient` desktop bridge regression proves the path does not open legacy task stores and does not issue a follow-up snapshot reread just to observe its own metadata write.

- 2026-06-09: Core action-start task notifications now carry sanitized `eventId` and `eventStatus` metadata, and the runtime-backed product store uses that metadata to suppress only the redundant self-accepted `in_progress` action-start notification it has already patched locally. Untagged task updates and terminal event-status updates still refresh cached task detail, so fast completions do not get hidden. A real `RuntimeServer`/`OpenADEClient` product-store regression starts an existing-task turn, proves the tagged start notification does not cause a follow-up task read, then sends a later completed update and proves the normal lightweight task refresh still runs.

- 2026-06-09: Existing-task `openade/turn/start` and `openade/review/start` now return enough accepted-result metadata for runtime-backed clients to avoid immediate task/snapshot rereads. Core, the TypeScript OpenADE module, generated OpenADE client contracts, `OpenADEClient`, `OpenADEProductStore`, and `CodeStore` now carry optional action `eventId`, `executionId`, and `createdAt` for turn/review start results. `OpenADEProductStore` patches cached in-progress action events, preview last-event state, working-task ids, and affected git-summary caches for existing tasks; unsupported source cases still use the conservative refresh path until the result carries enough preview data. Real product-store and desktop bridge regressions prove accepted existing-task starts update the observable task thread without follow-up `openade/task/read` or `openade/snapshot/read`.

- 2026-06-09: New-task `openade/turn/start` can now avoid immediate task/snapshot rereads when the accepted result returns the newly materialized `task` and `preview` DTOs. Go Core returns those DTOs for newly created turn-start tasks after the SQLite event/preview write, the TypeScript OpenADE module mirrors the shape through its real projection path, and `OpenADEProductStore` patches task cache, repo preview, and working-task ids from the accepted result while falling back to the old refresh path for older runtimes. Core action-start writes for turn, review, review follow-up, and queued-turn execution now set `UpdateLastEvent`/`UpdatePreview` so `task_previews` remains authoritative. A real `RuntimeServer`/`OpenADEClient` product-store regression starts a new task and proves no follow-up `openade/task/read` or `openade/snapshot/read` is needed to observe it.

- 2026-06-09: Classic desktop `ReviewPickerModal` now relies on the runtime-backed `CodeStore.startProductReview()` accepted-mutation cache sync instead of calling `refreshProductStateAfterTaskMutation()` after the user picks a review agent. A real `RuntimeServer`/`OpenADEClient` desktop bridge regression opens the picker modal, starts a plan review through the UI, and proves the in-progress review event appears without broad follow-up task or snapshot reads.

- 2026-06-09: Runtime-backed classic desktop task environment setup/prepare now follows the accepted-mutation cache-sync rule. `OpenADEProductStore.setupTaskEnvironment()` and `prepareTaskEnvironment()` patch cached device environments, setup events, preview last-event state, and scoped git/process/search caches after the runtime method succeeds instead of forcing follow-up task/snapshot reads. `CodeStore.prepareProductTaskEnvironment()` syncs the patched product-store cache into observable state, and the classic `EnvironmentSetupView` no longer calls `refreshProductStateAfterTaskMutation()` after Core accepts environment preparation. Real product-store and desktop bridge regressions prove direct setup emits no follow-up task/snapshot read, prepare performs only its bounded server-side task lookup, and the desktop setup view avoids legacy renderer worktree creation plus broad refreshes.

- 2026-06-09: Runtime-backed queued-turn cancellation, task usage materialization, and task deletion now patch accepted mutation results into cached DTO state without follow-up task/snapshot reads. `OpenADEProductStore.cancelQueuedTurn()` marks cached queued turns cancelled, usage backfill/recalculate patches preview usage from typed results, and task deletion removes cached task detail plus task previews/working-task ids locally. `CodeStore.deleteProductTask()` syncs the patched product-store snapshot, while runtime deletion cleanup now removes local task cache state without calling `openade/snapshot/read`. Real product-store and desktop bridge regressions prove these accepted mutations do not reread task detail or snapshot just to observe their own effects.

- 2026-06-09: Runtime-backed classic desktop title generation and comment create/edit/delete now avoid immediate task/snapshot rereads after the server accepts the mutation. `OpenADEProductStore.generateTaskTitle()` patches cached task and preview title DTOs from the typed result, comment mutations patch cached task comments locally, and `CodeStore` continues to sync that product-store cache into observable state. Real product-store and desktop bridge regressions prove accepted title/comment mutations do not issue hidden `openade/task/read` or `openade/snapshot/read` calls just to observe the mutation.

- 2026-06-09: Runtime-backed classic desktop repo mutations now follow the same accepted-mutation cache-sync rule as task mutations. `OpenADEProductStore.createRepo()`, `updateRepo()`, and `deleteRepo()` patch cached snapshot repo DTOs locally after the runtime method succeeds, `CodeStore` syncs that product-store cache into observable state, and `RepoManager` reserves `refreshProductStateAfterRepoMutation()` for legacy fallback or explicit external refresh. Real product-store and desktop bridge regressions prove accepted repo CRUD does not issue a follow-up `openade/snapshot/read` or legacy repo-store refresh.

- 2026-06-09: Runtime-backed classic desktop mutations now avoid immediate duplicate task/snapshot reads after the server accepts a mutation. `OpenADEProductStore.updateTaskMetadata()` patches cached task/preview DTOs locally for deterministic metadata fields instead of re-reading task detail and snapshot, while `CodeStore` mutation wrappers sync product-store DTO cache after turn start, review start, queued-turn cancel, task environment setup, and comment mutations. Runtime-backed managers now reserve broad `refreshProductStateAfterTaskMutation()` / `refreshProductStateAfterTaskCreation()` calls for legacy fallback or explicit external refreshes, and the real desktop route smoke no longer emits `openade/snapshot/read` / `openade/task/read` request-burst warnings during the classic workflow.
...
- 2026-06-09: Classic desktop runtime settlement now avoids loading uncached background task detail. `CodeStore` still clears runtime running state for terminal runtime notifications and still refreshes already-open/cached task DTOs before after-event scanning, but a background task that only appears in the snapshot no longer triggers a hidden `openade/task/read` just because its runtime completed. A real `RuntimeServer`/`OpenADEClient` bridge regression publishes running and completed runtime notifications for an unopened task and proves no task-detail read or after-event callback occurs.

- 2026-06-09: `OpenADEProductStore` no longer reads task detail for uncached background `openade/task/updated` or `openade/queuedTurn/updated` notifications. It still clears affected git-summary caches, still refreshes already-cached/open task detail after coalescing, and still treats `openade/task/previewChanged` / deletes as immediate snapshot-level invalidations. A real `RuntimeServer`/`OpenADEClient` product-store regression now publishes an uncached task update notification and proves it does not issue `openade/task/read`.
...
- 2026-06-09: `OpenADEProductStore.readProjectGitInfo()`, `readProjectGitBranches()`, `readProjectGitSummary()`, `readTaskGitSummary()`, `readTaskGitScopes()`, `readTaskGitLog()`, `readTaskGitCommitFiles()`, `readTaskGitFileAtTreeish()`, `readTaskGitCommitFilePatch()`, `readTaskChanges()`, `readTaskDiff()`, and `readTaskFilePair()` now keep one-second completed-result caches for duplicate task-open/tray churn while `TaskModel.refreshGitState({ force: true })` passes `bypassCache: true` so explicit summary refreshes still hit the selected runtime/Core. Repo update/delete clears project git-info and branch-list entries; scoped file writes, task git commits, repo update/delete, turn starts, and task update notifications clear affected repo/task git-summary, task-scope, task-log, commit-detail, changes, diff, and file-pair entries. Runtime-backed product-store tests prove duplicate project git-info, local/remote branch-list, project/task summary, task-scope, task-log, commit-files, file-at-treeish, commit-file-patch, changes, diff, and file-pair reads hit the runtime once per cache key, forced task refresh bypasses the cache, repo updates invalidate project git metadata, and scoped file writes plus task git commits invalidate affected task git caches.

- 2026-06-09: `OpenADEProductStore.readTaskSnapshotPatch()`, `readTaskSnapshotIndex()`, and `readTaskSnapshotPatchSlice()` now keep one-second completed-result caches for duplicate snapshot/diff viewer churn using compact repo/task/event/range keys rather than serializing snapshot event payloads. Task and repo deletion clear these immutable artifact caches so deleted tasks cannot serve stale patches. A real `RuntimeServer`/`OpenADEClient` product-store regression proves duplicate patch/index/slice reads hit the runtime once and that the same reads reject after task deletion instead of returning cached artifact data.

- 2026-06-09: `OpenADEProductStore.readMcpServers()` and `readPersonalSettings()` now keep one-second completed-result caches for duplicate startup/settings sync churn. Accepted MCP replace/upsert/delete and personal-settings replace results patch those caches locally, so runtime-backed desktop settings can observe accepted writes without immediately rereading `openade/settings/*` or falling back to legacy `code:mcp_servers` / `code:personal_settings` documents. A real `RuntimeServer`/`OpenADEClient` product-store regression proves duplicate settings reads hit the runtime once and accepted settings writes update subsequent reads without another runtime read.

- 2026-06-09: `OpenADEProductStore.readTaskImage()` and `readStagedTaskImage()` now keep one-second completed-result caches for duplicate classic thumbnail/shared-image render churn. Task-image cache entries clear on task/repo deletion and task update notifications, while accepted `writeTaskImage()` results patch staged-image cache entries from the uploaded bytes so SmartEditor preview restore does not immediately reread `openade/task/image/staged/read`. A real `RuntimeServer`/`OpenADEClient` product-store regression proves duplicate task/staged image reads hit the runtime once, deleted tasks do not serve cached task-image data, and accepted image writes update staged reads without another runtime request.
...
- 2026-06-09: `OpenADEProductStore.listProjectFiles()` and `readProjectFile()` now keep one-second completed-result caches for identical scoped repo/task tree and file-content reads, using the same scoped invalidation family as fuzzy/content search. Scoped file writes, task environment setup, task update notifications, task deletion, and repo update/delete clear affected tree/file/search entries so classic Files/Search tray churn does not repeatedly call `openade/project/files/tree` or `openade/project/file/read` while still observing real worktree changes. A real `RuntimeServer`/`OpenADEClient` product-store regression proves back-to-back tree/file reads hit the runtime once and hit it again after a scoped file write.
...
- 2026-06-09: `OpenADEProductStore.listRuntimes()` now keeps a one-second completed-result cache for identical runtime-list filters, covering duplicate clean-Core active-task hydration reads after in-flight coalescing resolves. Accepted runtime lifecycle mutations clear the completed-read cache immediately, and runtime lifecycle notifications still merge into `RuntimeRecordCache` immediately while clearing the cache before the next list call, so running-state changes are not hidden. A real `RuntimeServer`/`OpenADEClient` product-store regression seeds actual runtime supervisor records, proves duplicate active-runtime list reads hit `runtime/list` once, proves an accepted turn start clears the cache before any notification, publishes a real `runtime/updated` notification, and proves the next list hits the runtime again with the new active record.
...
- 2026-06-10: The generated OpenADE client contract now includes `openade/task/list` with a typed `OpenADEClient.listTasks(repoId)` helper, closing the remaining generated-client drift against Core/TypeScript task-list registrations. The live generated-contract test now calls task list over the real Go Core WebSocket for both trusted and paired clients after creating/starting tasks, so shared shell task-list capability remains verified through the same remote-kernel path instead of a mocked runtime.

- 2026-06-10: Remote/shared shell project opens now refresh selected-project task previews through scoped `openade/task/list` when the capability is advertised, instead of rereading the full snapshot for project-local freshness. `OpenADEProductStore.listTasks()` updates cached project/snapshot previews from the focused route, RemoteApp falls back to snapshot refresh for older runtimes, and task delete uses task-list refresh before snapshot repair. Focused real `RuntimeServer`/`OpenADEClient` tests prove the product store issues `openade/task/list` without another snapshot read and the RemoteApp project-open path uses the focused task-list read.

- 2026-06-10: The generated OpenADE client contract now owns product/remote notification names alongside method names and request/response DTO mappings. The contract generator verifies those notifications exist in both Go Core and the TypeScript OpenADE module, and the live generated-client Core WebSocket test asserts trusted initialize capabilities advertise them while paired initialize capabilities still hide unrelated runtime lifecycle notifications. Runtime-level `connection/*` notifications remain owned by the runtime protocol rather than the OpenADE product contract.

- 2026-06-10: The generated OpenADE client contract now owns the literal JSON-RPC/runtime error-code set used by Core, the TypeScript runtime, and the TypeScript OpenADE module. `npm run check:contracts` scans non-test implementation sources for `RuntimeError`, `runtimeError`, and `RuntimeHandlerError` construction sites and fails if a new public runtime error code is not added to `openade-contracts.json`; the live generated-client Core WebSocket test also asserts real `permission_denied` and `method_not_found` failures are members of the generated error-code contract. This is the next contract-first step before a full shared schema generator for every request/response validator.

- 2026-06-10: The generated OpenADE client contract now validates DTO type references before rendering. `npm run check:contracts` verifies every `imports` entry in `openade-contracts.json` is actually exported from `projects/openade-module/src/types.ts`, and every capitalized type referenced by a local/request/response mapping is either declared in that method's imports, defined in `localTypes`, or an allowed TypeScript built-in. This keeps request/response contract edits from silently depending on missing or undeclared DTO aliases while the full shared schema generator is still being designed.

- 2026-06-10: `OpenADEClient` now uses a generated OpenADE method overload on its runtime transport boundary. `RuntimeClientLike.request()` still has a raw runtime fallback for methods such as `runtime/list`, but product calls from `OpenADEClient` go through the generated `OpenADERequestForMethod` / `OpenADEResponseForMethod` overload and preserve the generated optional-params tuple. A focused OpenADE client test proves no-params product reads omit explicit `undefined` params while raw runtime calls keep their params shape.
...
- 2026-06-09: `OpenADEProductStore.readCronInstallState()` now keeps a one-second completed-result cache per repo, and accepted `replaceCronInstallState()` results patch that cache locally. Repo update/delete notifications clear the affected repo cache along with other scoped product read caches. A real `RuntimeServer`/`OpenADEClient` product-store regression proves duplicate cron install-state reads hit the runtime once and accepted replacements are visible without immediately rereading `openade/cron/installState/read`.
...
- 2026-06-09: `OpenADEProductStore.readTaskResourceInventory()` now keeps a one-second completed-result cache per task for duplicate classic delete/resource inventory reads. Task metadata/title changes, task updates/deletion, environment setup, turn/review starts, repo update/delete, and runtime lifecycle notifications clear the affected entries because the DTO includes durable artifacts plus dynamic `isRunning`. A real `RuntimeServer`/`OpenADEClient` product-store regression proves duplicate inventory reads hit the runtime once, accepted metadata changes refresh the task title, and a real `runtime/updated` notification causes the next inventory read to observe `isRunning: true`.
