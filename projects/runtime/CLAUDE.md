...
- RuntimeServer may receive product/host-owned `onSlowRequest` and `onNotificationBurst` observers. Keep those hooks generic and sanitized: slow requests may include service name, method name, sanitized/truncated request id, total duration, queue wait, handler time, a timing-derived dominant phase (`queue_wait`, `handler`, or `mixed`), connection id, failed flag, and error code; notification bursts may include service name, notification method, count, and window duration. Malformed JSON and invalid runtime request envelopes should emit slow-request events as `protocol/decode` when they cross the threshold, without logging raw payloads. Request params, notification params, file paths, prompts, task content, tokens, raw unbounded client-controlled ids, or product-specific payload fields are not allowed in these observer events.
