...
- Image prompt assembly, trusted-local `openade/task/image/write` compatibility storage, and enabled MCP server config assembly live in Electron main host adapter code before runtime-owned harness starts. Paired devices may still read scoped task images only through `openade/task/image/read` until upload permissions are explicitly reviewed.
