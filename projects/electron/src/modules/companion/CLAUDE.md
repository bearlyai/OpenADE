...
- When `OPENADE_CORE_RUNTIME_URL` is configured, OpenADE Core is the pairing/device/runtime authority. Electron's companion server remains the private-network HTTP/WebSocket listener but proxies `/v1/pair` and `/v1/runtime` to Core instead of using the legacy Electron device store/runtime.
...
- Core companion bridge: coreBridge.ts
...
- Runtime adapter inputs that cross from broad JSON/runtime records into concrete Electron host APIs must be narrowed with local validators instead of `as unknown as` assertions. In particular, MCP config and harness prompt/options/tool-result adapters should reject malformed payloads precisely while preserving expected executor-level failures such as unknown harness ids returning `{ ok: false }` and settling the runtime record.
...
- Running-task queueing must preserve the requested turn type and payload. Do/Ask/HyperPlan queued turns may be persisted through `openade/queued-turn/enqueue`; HyperPlan queued turns must carry the same serialized `hyperplanStrategy` that `openade/turn/start` requires.
...
- Electron companion product notification emits in `runtimeGateway.ts` and `deviceRuntime.ts` must import generated `OPENADE_NOTIFICATION` constants from `projects/openade-client`; `npm run check:contracts` rejects copied concrete `openade/*` or `remote/device/changed` notification strings in those emits.
...
- `runtimeGateway.ts` installs RuntimeServer's sanitized handler context wrapper so Yjs storage slow/repeated-load logs can show the active runtime method and storage operation. Preserve that context when refactoring registration; do not replace it with raw request params, stack traces, prompts, paths, or payload logging.
...
- Image prompt assembly, scoped `openade/task/image/write` compatibility storage, and enabled MCP server config assembly live in Electron main host adapter code before runtime-owned harness starts. Paired devices may read task-owned images through `openade/task/image/read` and upload composer images through `openade/task/image/write`; staged-image reads, image imports, image GC, raw file writes, and host paths remain trusted-local only.
...
- Paired-device method and notification grants are contract-owned in `projects/openade-client/openade-contracts.json`, generated into TypeScript/Go, and re-exported from `projects/shared/companion/src/runtimePermissions.ts` as `COMPANION_RUNTIME_PERMISSIONS` and `COMPANION_RUNTIME_NOTIFICATION_PERMISSIONS`; Electron `runtimeSocket.ts`, shared-shell tests, and companion integration tests must consume those constants instead of carrying local copies. Concrete `openade/*` product grants, `remote/device/*` session grants/registrations, and `remote/device/changed` must use generated `OPENADE_METHOD` / `OPENADE_REMOTE_METHOD` / `OPENADE_NOTIFICATION` constants from `projects/openade-client`; `npm run check:contracts` in `projects/openade-client` rejects copied product/remote strings in this file and fails if the companion paired profile stops using the generated contract-owned profile. The explicit `openade/*` notification wildcard remains a reviewed paired-profile policy.
...
- Paired devices may use high-level product mutations intentionally surfaced in the companion UI: task create, turn start/interrupt, review start, queued-turn enqueue/reorder/cancel, comment create/edit/delete, task metadata update, scoped task image write, and task delete.
- Paired devices may use reviewed scoped project and task read methods such as `openade/project/files/tree`, `openade/project/file/read`, `openade/project/search`, `openade/project/git/info/read`, `openade/project/git/branches/read`, `openade/project/git/summary/read`, `openade/project/process/list`, `openade/project/process/reconnect`, `openade/cron/definitions/read`, `openade/settings/mcpServers/read`, `openade/task/changes/read`, `openade/task/diff/read`, `openade/task/filePair/read`, `openade/task/git/summary/read`, `openade/task/git/scopes/read`, `openade/task/git/log`, `openade/task/git/commit/files/read`, `openade/task/git/fileAtTreeish/read`, `openade/task/git/commit/filePatch/read`, `openade/task/image/read`, `openade/task/resourceInventory/read`, and `openade/task/snapshot/*` reads. These methods must resolve repo/task paths server-side and must not become shortcuts to raw filesystem, git, process, PTY, snapshot storage, host, or Yjs/data access. Scoped project git reads may expose repo-root-equivalent paths and branch/status metadata already implied by OpenADE project DTOs, but raw `git/*` must remain denied. Scoped git scope reads must not expose raw worktree paths, and scoped git log scope ids must be resolved server-side. Scoped image reads must prove task ownership before loading bytes. Scoped MCP settings reads must return connector summaries only to read-only paired clients; they must not expose URLs, commands, args, cwd, headers, OAuth tokens, env vars, or settings-admin mutation powers. Scoped resource inventory may include task-owned snapshot ids, image ids, session ids, and worktree branch status, but must not expose raw host paths beyond the task/project DTOs already returned by OpenADE reads. `agent/provider/list`, `agent/provider/status`, `agent/serverProtocol/list`, `agent/approval/*`, `openade/repo/create`, `openade/repo/update`, `openade/repo/delete`, `openade/project/process/start`, `openade/project/process/stop`, `openade/project/file/write`, `openade/cron/run`, `openade/task/git/commit`, `openade/task/title/generate`, `openade/task/environment/prepare`, `openade/settings/mcpServers/replace`, `openade/settings/mcpServers/upsert`, `openade/settings/mcpServers/delete`, and `openade/task/terminal/*` exist for trusted/local product clients but must not be granted to paired devices without an explicit role/permission decision and matching integration tests.
- Trusted/local task-terminal reconnects may omit `terminalId`; `runtimeGateway.ts` must derive the scoped terminal id from `repoId` and `taskId` server-side and reject mismatched provided ids. Do not reintroduce renderer-side task PTY id derivation for the runtime-backed desktop tray path.
- Do not grant paired mobile devices direct `host/*` access. These trusted-local methods include platform probes, managed binaries, subprocess env, directory creation, MCP OAuth/test operations, Procs config editing, raw Yjs, blob data, snapshot storage, and Core legacy-migration acceptance markers.
- Generic `agent/*` provider, server-protocol, and approval methods are trusted/local only for now. Paired clients should use OpenADE product methods; exposing agent state remotely needs an explicit product role, Core parity, and real WebSocket tests.
...
- Paired mobile notification permissions should match the Core paired profile: `connection/*`, `remote/device/changed`, and `openade/*`. Do not use broad `remote/*`; future remote notifications may carry pairing/admin state.
- Low-level host methods may exist on the runtime server for trusted local/in-process clients.
- Main process owns CORS, network binding, and keep-awake. In legacy IPC sessions it also owns auth, pairing, device revocation, runtime auth, raw host powers, and Yjs-backed read projections.
- In Core sessions, do not call `auth.ts` pairing/device state or Electron's compatibility runtime for public companion clients. `coreBridge.ts` must proxy the public companion pair/runtime paths to Core, and trusted desktop device administration must use generated `OPENADE_REMOTE_METHOD` calls against the selected product runtime.
