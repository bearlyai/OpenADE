...
- Paired devices may use reviewed scoped project and task methods such as `openade/project/files/tree`, `openade/project/file/read`, `openade/project/search`, `openade/project/git/info/read`, `openade/project/git/branches/read`, `openade/project/git/summary/read`, `openade/project/process/*`, `openade/task/changes/read`, `openade/task/diff/read`, `openade/task/filePair/read`, `openade/task/git/scopes/read`, `openade/task/git/log`, `openade/task/image/read`, `openade/task/resourceInventory/read`, and `openade/task/snapshot/*` reads. These methods must resolve repo/task paths server-side and must not become shortcuts to raw filesystem, git, process, PTY, snapshot storage, host, or Yjs/data access. Scoped project git reads may expose repo-root-equivalent paths and branch/status metadata already implied by OpenADE project DTOs, but raw `git/*` must remain denied. Scoped git scope reads must not expose raw worktree paths, and scoped git log scope ids must be resolved server-side. Scoped image reads must prove task ownership before loading bytes. Scoped resource inventory may include task-owned snapshot ids, image ids, session ids, and worktree branch status, but must not expose raw host paths beyond the task/project DTOs already returned by OpenADE reads. Scoped process start may run only server-resolved `openade.toml` process definitions. `openade/project/file/write`, `openade/task/git/commit`, `openade/task/title/generate`, `openade/task/environment/prepare`, and `openade/task/terminal/*` exist for trusted/local product clients but must not be granted to paired devices without an explicit role/permission decision and matching integration tests.
