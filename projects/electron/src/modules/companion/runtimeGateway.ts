...
import {
	...
	type OpenADEProject,
	type OpenADEProjectSdkCapabilitiesReadRequest,
	...
	type OpenADEQueuedTurnReorderRequest,
		...
		type OpenADECronInstallStateListResult,
		type OpenADECronDefinitionsReadRequest,
		type OpenADECronDefinitionsReadResult,
...
import {
	listOrphanHarnessProcesses,
	terminateOrphanHarness,
} from "../code/orphanHarness";
import {
	deleteRuntimeDataFile,
	listRuntimeDataFileIds,
	loadRuntimeDataFile,
	runWithDataFolderOperationContext,
	saveRuntimeDataFile,
} from "../code/dataFolder";
import {
	deleteRuntimeBranch,
...
import {
	type SnapshotPatchFile,
	type SnapshotPatchIndex,
} from "../code/snapshotsIndex";
import {
	killRuntimePty,
	reconnectRuntimePty,
	resizeRuntimePty,
...
import {
	getRuntimeCodeCapabilities,
	getRuntimeSdkCapabilities,
	invalidateRuntimeSdkCapabilities,
...
import {
	registerRuntimeAgentModule,
	registerServerProtocolAgentBridge,
} from "./runtimeAgents";
...
import { markOpenADECoreLegacyYjsMigrationAcceptedFromUnknown, revokeOpenADECoreLegacyYjsMigrationAcceptance } from "../openadeCoreMigration";
...
const scopedProjectProcesses = new Map<
	string,
	OpenADEProjectProcessRegistration
>();
const SCOPED_PROJECT_PROCS_CACHE_TTL_MS = 10_000;
const RUNTIME_PROCESS_LIST_CACHE_TTL_MS = 15_000;
...
const scopedProjectProcsCache = new Map<
	string,
	PromiseCacheEntry<ReadProcsResult>
>();
let runtimeProcessListCache: PromiseCacheEntry<
	Awaited<ReturnType<typeof listRuntimeProcesses>>
> | null = null;
const quitBlockingRuntimeKinds = new Set([
	"agent",
	"process",
	"pty",
	"composite",
...
export function hasActiveRuntimeWork(): boolean {
	return (
		runtimeServer?.supervisor
			...
			.some(
				(runtime) =>
					quitBlockingRuntimeKinds.has(runtime.kind) &&
...
function queuedTurnIdForClientRequest(
	taskId: string,
	clientRequestId: string | undefined,
): string {
	return (
		openADEQueuedTurnIdForClientRequest(taskId, clientRequestId) ??
		`queued-${randomUUID()}`
	);
}
...
function queuedTurnParams(
	...
	return {
		...
		images: turn.images,
		hyperplanStrategy: turn.hyperplanStrategy,
...
function sessionBackedStreamEventId(
	executionId: string,
	index: number,
	event: unknown,
): string {
...
function sessionHarnessEventToStreamEvent(params: {
	executionId: string;
	harnessId: string;
	index: number;
	event: unknown;
	...
	if (type === "error") {
		return {
			...base,
			type: "error",
			error: typeof event.error === "string" ? event.error : "Session error",
			code: event.code,
		};
	}
...
function mergeSessionBackedStreamEvents(
...
async function hydrateOpenADETaskSessionEvents(params: {
	server: RuntimeServer;
	task: OpenADETask;
	repoPath?: string;
}): Promise<OpenADETask> {
	...
	for (const event of params.task.events) {
		...
		const harnessId =
			typeof execution.harnessId === "string" ? execution.harnessId : undefined;
		const executionId =
			typeof execution.executionId === "string"
				? execution.executionId
				: undefined;
		const sessionId =
			typeof execution.sessionId === "string" ? execution.sessionId : undefined;
		...
		if (harnessId && executionId && sessionId) {
			...
			const storedEvents = Array.isArray(execution.events)
				? execution.events
				: [];
			hydratedExecution = {
				...execution,
		...
		const hydratedSubExecutions = Array.isArray(record.hyperplanSubExecutions)
			? await Promise.all(
					record.hyperplanSubExecutions.map(async (subExecution) => {
						if (
							typeof subExecution !== "object" ||
							subExecution === null ||
							Array.isArray(subExecution)
						)
							return subExecution;
						const sub = subExecution as Record<string, unknown>;
						const subHarnessId =
							typeof sub.harnessId === "string" ? sub.harnessId : undefined;
						const subExecutionId =
							typeof sub.executionId === "string" ? sub.executionId : undefined;
						const subSessionId =
							typeof sub.sessionId === "string" ? sub.sessionId : undefined;
						if (!subHarnessId || !subExecutionId || !subSessionId)
							return subExecution;
						const sessionEvents = await readHarnessSessionStreamEvents({
							server: params.server,
						...
						return {
							...sub,
							events: mergeSessionBackedStreamEvents(
								storedEvents,
								sessionEvents,
							),
						};
		...
		hydratedEvents.push({
			...
			...(hydratedSubExecutions
				? { hyperplanSubExecutions: hydratedSubExecutions }
				: {}),
		});
	}
...
function registerConfiguredServerProtocolBridges(server: RuntimeServer): void {
	const codexUrl =
		process.env.OPENADE_CODEX_APP_SERVER_URL ??
		process.env.CODEX_APP_SERVER_URL;
	if (!codexUrl) return;

	const managedCommand =
		process.env.OPENADE_CODEX_APP_SERVER_COMMAND ??
		process.env.CODEX_APP_SERVER_COMMAND;
	const managedArgs =
		process.env.OPENADE_CODEX_APP_SERVER_ARGS_JSON ??
	...
	const bridge = createRuntimeNodeCodexAppServerBridge({
		...
		authToken:
			process.env.OPENADE_CODEX_APP_SERVER_TOKEN ??
			process.env.CODEX_APP_SERVER_TOKEN,
		clientName: "openade",
		clientVersion: process.env.RELEASE ?? "unknown",
		managedProcess: managedCommand
			? {
					command: managedCommand,
					args: parseStringArrayEnv(managedArgs) ?? [
						"app-server",
						"--listen",
						codexUrl,
					],
					cwd:
						process.env.OPENADE_CODEX_APP_SERVER_CWD ??
						process.env.CODEX_APP_SERVER_CWD,
					readyProbeUrl:
						process.env.OPENADE_CODEX_APP_SERVER_READY_URL ??
						process.env.CODEX_APP_SERVER_READY_URL,
				}
			: undefined,
...
function canExecuteTaskInRuntime(task: OpenADETask): boolean {
	...
	return (
		isolationStrategy.type === "head" || isolationStrategy.type === "worktree"
	);
}
...
function modelIdForTurn(
	...
	return (
		params.modelId ??
		lastActionSessionContext(task)?.modelId ??
		getDefaultModelForHarness(harnessId)
	);
}

function lastActionSessionContext(
	task: OpenADETask,
): { sessionId: string; harnessId?: string; modelId?: string } | null {
	for (let index = task.events.length - 1; index >= 0; index--) {
		...
		if (typeof execution.sessionId === "string" && execution.sessionId) {
			return {
				...
				harnessId:
					typeof execution.harnessId === "string"
						? execution.harnessId
						: undefined,
				modelId:
					typeof execution.modelId === "string" ? execution.modelId : undefined,
			};
		}
	}
	return null;
		...
		.map((image): RuntimeImageAttachment | null => {
			...
			const id =
				typeof record.id === "string" && /^[a-zA-Z0-9_-]+$/.test(record.id)
					...
					: undefined;
			const ext =
				typeof record.ext === "string" && /^[a-zA-Z0-9]+$/.test(record.ext)
					? record.ext
					: undefined;
			const mediaType =
...
function optionalRuntimeStringParam(
	record: Record<string, unknown>,
	key: string,
): string | undefined {
	const value = record[key];
...
function runtimeStringRecordParam(
	record: Record<string, unknown>,
	key: string,
	...
	for (const [recordKey, recordValue] of Object.entries(value)) {
		if (typeof recordValue !== "string")
			throw new Error(`${key}.${recordKey} is invalid`);
		result[recordKey] = recordValue;
	}
	return result;
}

function optionalRuntimeStringArrayParam(
	record: Record<string, unknown>,
	key: string,
): string[] | undefined {
	const value = record[key];
...
function optionalRuntimeStringRecordParam(
	record: Record<string, unknown>,
	key: string,
): Record<string, string> | undefined {
	const value = record[key];
	if (value === undefined) return undefined;
	return runtimeStringRecordParam(record, key);
}

function runtimeMcpServerConfigParam(value: unknown): RuntimeMcpServerConfig {
	const record = runtimeRecordParam(value);
	const type = runtimeStringParam(record, "type");
	if (type === "stdio") {
		return {
			type,
			command: runtimeStringParam(record, "command"),
			args: optionalRuntimeStringArrayParam(record, "args"),
			env: optionalRuntimeStringRecordParam(record, "env"),
			cwd: optionalRuntimeStringParam(record, "cwd"),
		};
	}
	if (type === "http") {
		return {
			type,
			url: runtimeStringParam(record, "url"),
			headers: optionalRuntimeStringRecordParam(record, "headers"),
		};
	}
	throw new Error("config.type is invalid");
}
...
function registerTrustedHostMethods(server: RuntimeServer): void {
	...
	server.register("host/binaries/ensure", async (params) => {
		...
		try {
			...
			console.error(
				`[RuntimeGateway] host/binaries/ensure(${name}) failed:`,
				message,
			);
	...
	server.register("host/binaries/resolve", (params) => ({
		path: resolveRuntimeBinary(
			runtimeStringParam(runtimeRecordParam(params), "name"),
		),
	}));
	server.register("host/platform/info", () => getRuntimePlatformInfo());
	server.register("host/system/checkBinary", (params) =>
		checkRuntimeBinary(
			runtimeStringParam(runtimeRecordParam(params), "binary"),
		),
	);
	server.register("host/system/checkVendoredRipgrep", () =>
		checkRuntimeVendoredRipgrep(),
	);
	...
	server.register("host/shell/createDirectory", (params) =>
		createRuntimeDirectory({
			path: runtimeStringParam(runtimeRecordParam(params), "path"),
		}),
	);
	server.register("host/mcp/testConnection", (params) => {
		const record = runtimeRecordParam(params);
		const config = runtimeMcpServerConfigParam(record.config);
	...
	server.register("host/mcp/cancelOAuth", (params) =>
		cancelRuntimeMcpOAuth({
			serverId: runtimeStringParam(runtimeRecordParam(params), "serverId"),
		}),
	);
	server.register("host/mcp/refreshOAuth", (params) => {
		const record = runtimeRecordParam(params);
			return refreshRuntimeMcpOAuth({
				...
				clientId: optionalRuntimeStringParam(record, "clientId"),
			});
		});
	server.register("host/procs/read", (params) =>
		readRuntimeProcs({
			path: runtimeStringParam(runtimeRecordParam(params), "path"),
		}),
	);
	server.register("host/procs/file/read", (params) =>
		readRuntimeProcsFile({
			filePath: runtimeStringParam(runtimeRecordParam(params), "filePath"),
		}),
	);
	...
	server.register("host/procs/editable/save", (params) => {
		...
		if (!Array.isArray(crons)) throw new Error("crons is invalid");
		invalidateScopedProjectProcessCaches(
			optionalRuntimeStringParam(record, "searchPath"),
		);
	...
	server.register("host/core/legacyYjsMigration/accept", (params) =>
		markOpenADECoreLegacyYjsMigrationAcceptedFromUnknown(params),
	);
	server.register("host/core/legacyYjsMigration/revoke", () =>
	...
	server.register("agent/sdkCapabilities/read", (params) => {
		...
		return getRuntimeSdkCapabilities({
			...
			harnessId: optionalRuntimeStringParam(record, "harnessId") as
				| HarnessId
				| undefined,
		});
	});
	...
	server.register("data/file/load", async (params) => {
		...
		return {
			data: (Buffer.isBuffer(data) ? data : Buffer.from(data, "utf8")).toString(
				"base64",
			),
		};
	});
	...
	server.register("snapshot/patch/read", (params) =>
		loadRuntimeSnapshotPatch({
			id: runtimeStringParam(runtimeRecordParam(params), "id"),
		}),
	);
	server.register("snapshot/index/read", (params) =>
		loadRuntimeSnapshotIndex({
			id: runtimeStringParam(runtimeRecordParam(params), "id"),
		}),
	);
	...
	server.register("snapshot/bundle/delete", (params) =>
		deleteRuntimeSnapshotBundle({
			id: runtimeStringParam(runtimeRecordParam(params), "id"),
		}),
	);
...
async function buildHarnessPrompt(
	text: string,
	images?: unknown[],
): Promise<string | HarnessContentBlock[]> {
	...
	for (const image of attachments) {
		try {
			...
			console.warn("[RuntimeGateway] Failed to attach image to prompt", {
				imageId: image.id,
				error,
			});
		}
...
async function cleanupTaskResources(
	task: OpenADETask,
	repoPath: string,
	options: NonNullable<OpenADETaskDeleteRequest["options"]>,
): Promise<void> {
	const active = activeTaskExecutions.get(task.id);
	const inventory = buildOpenADETaskResourceInventory({
		task,
	...
	if (options.deleteSnapshots) {
		await Promise.all(
			inventory.snapshotIds.map((id) =>
				deleteRuntimeSnapshotBundle({ id }).catch(() => undefined),
			),
		);
	}
	...
	if (options.deleteSessions) {
		await Promise.all(
			inventory.sessions.map((session) =>
				deleteRuntimeHarnessSession(session).catch(() => ({ ok: false })),
			),
		);
	}

	if (options.deleteWorktrees && task.isolationStrategy?.type === "worktree") {
		const gitInfo = await isRuntimeGitDirectory({ directory: repoPath }).catch(
			() => null,
		);
		if (gitInfo?.isGitDirectory) {
			await deleteRuntimeWorkTree({
				repoDir: gitInfo.repoRoot,
				id: task.slug,
			}).catch(() => undefined);
			await deleteRuntimeBranch({
				repoDir: gitInfo.repoRoot,
...
function latestScopedTaskEnvironment(
	task: OpenADETask,
): OpenADETaskDeviceEnvironment | undefined {
	for (let index = task.deviceEnvironments.length - 1; index >= 0; index--) {
		const environment = task.deviceEnvironments[index];
		if (environment.setupComplete && environment.worktreeDir)
			return environment;
	}
	return undefined;
}

async function scopedTaskWorkDir(
	repo: OpenADEProject,
	task: OpenADETask,
): Promise<string> {
...
function scopedTaskFromTreeish(
	task: OpenADETask,
	fromTreeish?: string,
): string {
	if (fromTreeish) return fromTreeish;
	return (
		snapshotBaseForTask(task, latestScopedTaskEnvironment(task))?.fromTreeish ??
		"HEAD"
	);
}
...
function scopedSdkCapabilitiesHarnessId(value: string | undefined): HarnessId {
	if (value === undefined || value.trim() === "") return "claude-code";
	if (value === "claude-code" || value === "codex") return value;
	throw new Error("harnessId is unsupported");
}

async function readScopedProjectSdkCapabilities(
	params: OpenADEProjectSdkCapabilitiesReadRequest & {
		repo: OpenADEProject;
		task?: OpenADETask;
	},
): Promise<OpenADESdkCapabilities> {
	const cwd = params.task
		? await scopedTaskWorkDir(params.repo, params.task)
		: params.repo.path;
	const capabilities = await getRuntimeSdkCapabilities({
		cwd,
		harnessId: scopedSdkCapabilitiesHarnessId(params.harnessId),
	});
	return (
		capabilities ?? {
			slash_commands: [],
...
async function readScopedProjectGitBranches(
	...
	const result = await listRuntimeBranches({
		repoDir: gitInfo.repoRoot,
...
async function readScopedProjectGitSummary(
	...
	const stagedFiles = summary.staged.files.map((file) =>
		scopedTaskSummaryFile(file, "modified"),
	);
	const unstagedFiles = summary.unstaged.files.map((file) =>
		scopedTaskSummaryFile(file, "modified"),
	);
	const untracked = summary.untracked.map((file) =>
		scopedTaskSummaryFile(file, "added"),
	);
	return {
		repoId: params.repoId,
...
async function readScopedTaskGitScopes(
	...
	const [branches, worktrees] = await Promise.all([
		...
		listRuntimeWorkTrees({ repoDir: workDir }).catch(() => ({
			worktrees: [],
		})),
	]);
...
async function scopedTaskGitWorkDirForLog(
	...
	const worktrees = await listRuntimeWorkTrees({
		repoDir: defaultWorkDir,
	}).catch(() => ({ worktrees: [] }));
	return (
		worktrees.worktrees.find((worktree) => worktree.id === worktreeId)?.path ??
		defaultWorkDir
	);
}
...
async function readScopedTaskResourceInventory(
	...
	if (params.task.isolationStrategy?.type === "worktree") {
		const gitInfo = await isRuntimeGitDirectory({
			directory: params.repo.path,
		}).catch(() => null);
		if (gitInfo?.isGitDirectory) {
...
async function readScopedTaskChanges(
	...
	const result = await getRuntimeChangedFiles({
		workDir,
		fromTreeish,
		toTreeish: "",
	});
...
async function readScopedTaskDiff(
...
async function readScopedTaskGitLog(
	params: OpenADETaskGitLogRequest & {
		repo: OpenADEProject;
		task: OpenADETask;
	},
...
async function readScopedTaskGitCommitFiles(
	...
	const workDir = await scopedTaskWorkDir(params.repo, params.task);
	const result = await getRuntimeCommitFiles({
		workDir,
...
async function readScopedTaskGitFileAtTreeish(
	...
	const workDir = await scopedTaskWorkDir(params.repo, params.task);
	const result = await getRuntimeFileAtTreeish({
		workDir,
...
async function commitScopedTaskGit(
	...
	const workDir = await scopedTaskWorkDir(params.repo, params.task);
	const result = await commitRuntimeWorkingTree({
		workDir,
...
async function generateScopedTaskTitle(
	...
	const harnessId = (params.harnessId ??
		lastActionSessionContext(params.task)?.harnessId ??
		"claude-code") as HarnessId;
	const prompt = buildOpenADETaskTitlePrompt({
		description,
		events: params.task.events,
	});
...
const runtimeSnapshotPatchStore = {
	loadPatch: (patchFileId: string) =>
		loadRuntimeSnapshotPatch({ id: patchFileId }),
	loadIndex: (patchFileId: string) =>
		loadRuntimeSnapshotIndex({ id: patchFileId }),
	loadPatchSlice: (patchFileId: string, start: number, end: number) =>
		loadRuntimeSnapshotPatchSlice({ id: patchFileId, start, end }),
};
...
async function readScopedTaskSnapshotPatch(
	...
	return readOpenADETaskSnapshotPatch({
		...params,
		store: runtimeSnapshotPatchStore,
	});
}
...
async function readScopedTaskSnapshotIndex(
	...
	return readOpenADETaskSnapshotIndex({
		...params,
		store: runtimeSnapshotPatchStore,
	});
}
...
async function readScopedTaskSnapshotPatchSlice(
	...
	return readOpenADETaskSnapshotPatchSlice({
		...params,
		store: runtimeSnapshotPatchStore,
	});
}
...
async function readScopedTaskImage(
	...
	return {
		...
		data:
			data === null
				? null
				: (Buffer.isBuffer(data) ? data : Buffer.from(data, "utf8")).toString(
						"base64",
					),
	};
}

async function writeRuntimeTaskImage(
	params: OpenADETaskImageWriteRequest,
): Promise<OpenADETaskImageWriteResult> {
...
async function scopedProjectProcessSearchRoot(params: {
	repo: OpenADEProject;
	task?: OpenADETask;
}): Promise<string> {
	return params.task
...
function projectProcessDefinitionsFromProcs(result: ReadProcsResult): {
	...
	const root =
		result.isWorktree && result.worktreeRoot
			? result.worktreeRoot
			: result.repoRoot;
	return buildOpenADEProjectProcessDefinitions({
		root,
		configs: result.configs,
	});
}

function cachedScopedProjectProcs(
	searchRoot: string,
): Promise<ReadProcsResult> {
	const now = Date.now();
	const cached = scopedProjectProcsCache.get(searchRoot);
	if (cached?.promise) return cached.promise;
	if (cached?.result && cached.expiresAt && cached.expiresAt > now)
		return Promise.resolve(cached.result);

	const promise = readRuntimeProcs({ path: searchRoot })
		.then((result) => {
			scopedProjectProcsCache.set(searchRoot, {
				result,
				expiresAt: Date.now() + SCOPED_PROJECT_PROCS_CACHE_TTL_MS,
			});
			return result;
		})
		.catch((error: unknown) => {
			if (scopedProjectProcsCache.get(searchRoot)?.promise === promise)
...
function cachedRuntimeProcessList(): Promise<
	Awaited<ReturnType<typeof listRuntimeProcesses>>
> {
	const now = Date.now();
	if (runtimeProcessListCache?.promise) return runtimeProcessListCache.promise;
	if (
		runtimeProcessListCache?.result &&
		runtimeProcessListCache.expiresAt &&
		runtimeProcessListCache.expiresAt > now
	) {
	...
	const promise = listRuntimeProcesses()
		.then((result) => {
			runtimeProcessListCache = {
				result,
				expiresAt: Date.now() + RUNTIME_PROCESS_LIST_CACHE_TTL_MS,
			};
			return result;
		})
		.catch((error: unknown) => {
			if (runtimeProcessListCache?.promise === promise)
...
async function readScopedCronDefinitions(
	...
	return {
		...
		configs: procs.configs.map((config) => ({
...
async function listScopedProjectProcesses(
	...
	const instances = runtimeProcesses.processes
		.map((processInfo) => {
			...
			return registration &&
				openADEProjectProcessScopeMatches(registration, params)
				? openADEProjectProcessInstanceFromRuntimeInfo(
						processInfo,
						registration,
					)
		...
		.filter(
			(instance): instance is OpenADEProjectProcessInstance =>
				instance !== null,
		);
...
async function startScopedProjectProcess(
	...
	const processDef = listed.processes.find(
		(candidate) => candidate.id === params.definitionId,
	);
	if (!processDef)
		throw new Error(`Process definition ${params.definitionId} not found`);
...
async function writeScopedTaskTerminal(
	...
	const result = await writeRuntimePty({
		ptyId: params.terminalId,
		data: encodeOpenADETaskTerminalInput(params.data),
	});
	return {
		repoId: params.repoId,
		taskId: params.taskId,
...
async function resizeScopedTaskTerminal(
	...
	assertOpenADETaskTerminalId(params);
	const result = await resizeRuntimePty({
		ptyId: params.terminalId,
		cols: params.cols,
		rows: params.rows,
	});
...
async function buildRuntimeMcpServerConfigs(
	storage: OpenADEYjsStorage,
	...
	const rows =
		(await storage.readOrderedArray<Record<string, unknown>>(
			"code:mcp_servers",
			"mcp_servers",
		)) ?? [];
		...
		if (row.transportType === "http" && typeof row.url === "string") {
			const headers =
				...
				!Array.isArray(row.headers)
					...
					: {};
			const oauthTokens =
				typeof row.oauthTokens === "object" && row.oauthTokens !== null
					? (row.oauthTokens as Record<string, unknown>)
					: {};
			if (
				typeof oauthTokens.accessToken === "string" &&
			...
			configs[name] =
				Object.keys(headers).length > 0
					? { type: "http", url: row.url, headers }
					: { type: "http", url: row.url };
		} else if (
			row.transportType === "stdio" &&
			typeof row.command === "string"
		) {
			const config: Extract<McpServerConfig, { type: "stdio" }> = {
				type: "stdio",
				command: row.command,
			};
			if (Array.isArray(row.args))
				config.args = row.args.filter(
					(arg): arg is string => typeof arg === "string",
				);
			if (typeof row.cwd === "string" && row.cwd) config.cwd = row.cwd;
			if (
				typeof row.envVars === "object" &&
				row.envVars !== null &&
				!Array.isArray(row.envVars)
			) {
...
function publishTaskChanged(
	...
	repoId: string,
	taskId: string,
	options: { previewChanged?: boolean } = {},
): void {
...
function publishQueuedTurnUpdated(
	server: RuntimeServer,
	repoId: string,
	taskId: string,
	turn: OpenADEQueuedTurn,
...
async function enqueueDoTurn(params: {
	...
	if (
		params.turn.type !== "do" &&
		params.turn.type !== "ask" &&
		params.turn.type !== "hyperplan"
	) {
		throw new Error(
			"Only Do, Ask, and HyperPlan turns can be queued while another turn is running",
		);
	}
	...
	const existing = params.task.queuedTurns?.find(
		(turn) =>
			...
			(queuedTurn.clientRequestId &&
				turn.clientRequestId === queuedTurn.clientRequestId),
	);
	if (existing) {
		publishQueuedTurnUpdated(
			params.server,
...
async function enqueueQueuedTurn(params: {
	...
	const task = await params.projection.readTask(
		params.request.repoId,
		params.request.taskId,
	);
	const now = new Date().toISOString();
	const createdAt = params.request.createdAt ?? now;
	const queuedTurnId =
		params.request.queuedTurnId ??
		queuedTurnIdForClientRequest(
			params.request.taskId,
			params.request.clientRequestId,
		);
	const existing = task.queuedTurns?.find(
		(turn) =>
			turn.id === queuedTurnId ||
			(params.request.clientRequestId &&
				turn.clientRequestId === params.request.clientRequestId),
	);
	if (existing) {
		publishQueuedTurnUpdated(
			params.server,
	...
	const turn: OpenADEQueuedTurn = {
		...
		hyperplanStrategy: params.request.hyperplanStrategy,
...
async function reorderQueuedTurns(params: {
	...
	const turnsById = new Map(
		(task.queuedTurns ?? []).map((turn) => [turn.id, turn]),
	);
	...
	const nextTurns = [
		...requestedTurns,
		...(task.queuedTurns ?? []).filter((turn) => !requestedIds.has(turn.id)),
	];
	const previousOrder = (task.queuedTurns ?? [])
		.map((turn) => turn.id)
		.join("\0");
	...
	try {
		...
		if (next) {
			await updateQueuedTurn({
				...
				console.warn(
					"[RuntimeGateway] Failed to mark queued turn as errored",
					updateError,
				);
			});
		}
...
async function reconcileCheckpointedOpenADEActionEvents(
	...
	for (const runtime of server.supervisor.list({ ownerType: "openade-task" })) {
		...
		const eventId =
			typeof labels.eventId === "string" ? labels.eventId : undefined;
		const executionId =
			typeof labels.executionId === "string"
				? labels.executionId
				: runtime.nativeId;
		if (!eventId && !executionId) continue;

		const result = await writer
			.reconcileActionEventRuntime({
				taskId,
				eventId,
				executionId,
				status:
			...
			.catch((error) => {
				console.warn(
					"[RuntimeGateway] Failed to reconcile checkpointed OpenADE runtime",
					{ runtimeId: runtime.runtimeId, error },
				);
				return null;
			});
		if (result?.changed && result.repoId)
			publishTaskChanged(server, result.repoId, taskId);
	}
}
...
function inProgressActionEventIds(task: OpenADETask): string[] {
	...
	for (const rawEvent of task.events) {
		...
		if (!event || event.type !== "action" || event.status !== "in_progress")
			continue;
...
async function reconcileStaleTaskExecution(params: {
	...
	try {
		...
		if (!repoId) {
			...
			repoId = projects.find((project) =>
				project.tasks.some((task) => task.id === params.taskId),
			)?.id;
		}
		if (repoId) {
			const task = await params.projection.readTask(repoId, params.taskId);
			for (const eventId of inProgressActionEventIds(task)) {
				const result = await params.writer.reconcileActionEventRuntime({
					taskId: params.taskId,
					eventId,
					status: "stopped",
				});
		...
		console.warn("[RuntimeGateway] Failed to reconcile stale task execution", {
			taskId: params.taskId,
			error,
		});
	}
...
async function stopActiveOpenADERuntime(
	...
	if (
		runtime.scope.ownerType !== "openade-task" &&
		runtime.scope.ownerType !== "openade-turn" &&
		runtime.scope.ownerType !== "openade-review"
	)
		return false;
	const activeEntry = [...activeTaskExecutions.entries()].find(
		([, active]) => active.runtimeId === runtime.runtimeId,
	);
	...
	if (failed && typeof failed === "object" && "error" in failed) {
		throw new RuntimeHandlerError(
			...
			typeof failed.error === "string"
				? failed.error
				: "Failed to stop OpenADE runtime",
			{
...
function latestCompletedPlanEvent(
	task: OpenADETask,
...
function recentSnapshotFiles(task: OpenADETask, limit = 40): string[] {
	...
	for (
		let index = task.events.length - 1;
		index >= 0 && summaries.length < limit;
		index--
	) {
		const event = task.events[index];
		if (typeof event !== "object" || event === null || Array.isArray(event))
...
async function latestSnapshotPatch(
	task: OpenADETask,
): Promise<string | undefined> {
	const snapshot = latestSnapshotEvent(task);
	if (!snapshot) return undefined;
	if (typeof snapshot.fullPatch === "string" && snapshot.fullPatch.length > 0)
		return snapshot.fullPatch;
	if (
		typeof snapshot.patchFileId === "string" &&
		snapshot.patchFileId.length > 0
	) {
		return (
			(await loadRuntimeSnapshotPatch({ id: snapshot.patchFileId })) ??
			undefined
		);
	}
	return undefined;
...
async function createSnapshotForCompletedTurn({
	...
	try {
		const patchResult = await buildSnapshotPatch(
			rootPath,
			snapshotBase.fromTreeish,
		);
		if (
			patchResult.stats.filesChanged === 0 &&
			patchResult.stats.insertions === 0 &&
			patchResult.stats.deletions === 0
		)
			return false;
		...
		try {
			await saveRuntimeSnapshotBundle({
				id: snapshotEventId,
			...
			console.warn(
				"[RuntimeGateway] Failed to save snapshot patch bundle; storing patch inline:",
				error,
			);
		}
		...
		console.warn(
			"[RuntimeGateway] Failed to create snapshot for completed turn:",
			{
				taskId: task.id,
				eventId,
				error,
...
async function createTaskEnvironment({
	...
	const sourceBranch =
		isolationStrategy.sourceBranch || gitInfo.mainBranch || "main";
	...
	try {
		...
		console.warn(
			"[RuntimeGateway] Failed to resolve worktree merge base:",
			error,
		);
	}

	const workingDir = gitInfo.relativePath
		? `${worktree.worktreeDir}/${gitInfo.relativePath}`
...
async function ensureTaskExecutionEnvironment({
	...
	const deviceId = getDeviceConfig().deviceId;
	const existing = task.deviceEnvironments.find(
		(environment) =>
	...
	if (existing?.worktreeDir) {
		...
		return {
			cwd: relativePath
				? `${existing.worktreeDir}/${relativePath}`
...
function runtimeCronInstallStateFromUnknown(
	key: string,
	value: unknown,
): OpenADECronInstallState | null {
	if (typeof value !== "object" || value === null || Array.isArray(value))
		return null;
	const record = value as Record<string, unknown>;
	const cronId =
		typeof record.cronId === "string" && record.cronId.trim()
			? record.cronId.trim()
			: key;
	const installedAt =
	...
	return {
		...
		lastRunAt:
			typeof record.lastRunAt === "string" && record.lastRunAt
				...
				: undefined,
		lastTaskId:
			typeof record.lastTaskId === "string" && record.lastTaskId
				? record.lastTaskId
				: undefined,
	};
}

function parseRuntimeCronInstallStateDocument(
	raw: string,
): RuntimeCronInstallStateDocument {
	let parsed: unknown;
	try {
	...
	const source =
		typeof document.installations === "object" &&
		document.installations !== null &&
...
async function readRuntimeCronInstallState(
	params: OpenADECronInstallStateReadRequest,
): Promise<OpenADECronInstallStateReadResult> {
	const data = await loadRuntimeDataFile({
		folder: "cron",
		id: params.repoId,
		ext: "json",
	...
	return {
		repoId: params.repoId,
		installations: parseRuntimeCronInstallStateDocument(raw).installations,
	};
}

async function replaceRuntimeCronInstallState(
	params: OpenADECronInstallStateReplaceRequest,
): Promise<OpenADECronInstallStateReplaceResult> {
	const document: RuntimeCronInstallStateDocument = {
...
async function listRuntimeCronInstallStateRepos(): Promise<OpenADECronInstallStateListResult> {
	const ids = await listRuntimeDataFileIds({ folder: "cron", ext: "json" });
	const repoIds: string[] = [];
	for (const repoId of ids) {
		if (repoId === "_index") continue;
		const state = await readRuntimeCronInstallState({ repoId });
...
function registerOpenADEProductModule(server: RuntimeServer): void {
	const yjsStorage = createOpenADEYjsStorageAdapter({
	...
	server.registerRuntimeStopHandler((runtime) =>
		stopActiveOpenADERuntime(server, writer, runtime),
	);
	...
	server.registerModule(
		createOpenADEModule({
			...
			readTask: async (repoId, taskId, options) => {
				...
				if (options?.hydrateSessionEvents !== true) return task;
			...
			replaceMcpServers: (params) =>
				replaceYjsMcpServers(yjsStorage, params.servers),
			upsertMcpServer: (params) =>
				upsertYjsMcpServer(yjsStorage, params.server),
			deleteMcpServer: (params) =>
				deleteYjsMcpServer(yjsStorage, params.serverId),
			readPersonalSettings: () => readYjsPersonalSettings(yjsStorage),
			replacePersonalSettings: (params) =>
				replaceYjsPersonalSettings(yjsStorage, params.settings),
			readCronInstallState: readRuntimeCronInstallState,
			replaceCronInstallState: replaceRuntimeCronInstallState,
			listCronInstallStateRepos: listRuntimeCronInstallStateRepos,
			...
			scopedHost: {
				...
				readProjectSdkCapabilities: readScopedProjectSdkCapabilities,
				...
				readTaskGitCommitFiles: readScopedTaskGitCommitFiles,
					...
					readCronDefinitions: readScopedCronDefinitions,
			...
			saveDataDocumentBase64: (id, data, options) =>
				yjsStorage.saveDocumentUpdate(id, Buffer.from(data, "base64"), options),
			...
			startTurn: async (params, context) => {
				if (!canCreateTaskInRuntime(params)) {
					if (!params.inTaskId)
						throw new Error("Task id is required for existing task execution");
					const existingTask = await readTaskForMutation(
						projection,
						params.repoId,
						params.inTaskId,
					);
					if (!canExecuteTaskInRuntime(existingTask)) {
						throw new Error(
							"Server-owned execution supports head and worktree tasks only",
						);
					}
					...
					await reconcileStaleTaskExecution({
						server,
						writer,
						projection,
				...
				const repo = (await projection.readProjects()).find(
					(project) => project.id === params.repoId,
				);
				if (!repo) throw new Error(`Repository ${params.repoId} not found`);
				const createdAt = new Date().toISOString();
				const isolationStrategy = params.isolationStrategy ?? { type: "head" };
				const taskId = openADETaskIdForClientRequest(
					params.repoId,
			...
			interruptTurn: async (params) => {
				...
				const reconciled = await reconcileStaleTaskExecution({
					server,
					writer,
					projection,
					taskId: params.taskId,
				});
				if (reconciled.killed > 0 || reconciled.settled > 0)
					return { ok: true };
				return {
					ok: false,
					error: "No server-owned turn is running for this task",
				};
			},
			...
			deleteComment: async (params) => {
				...
				updateTaskMetadata: async (params) => {
					...
					if (params.closed) {
						await killRuntimePty({ ptyId: params.taskId }).catch(() => ({
							ok: false,
						}));
					}
...
async function startHeadModeTurn({
	...
	if (params.type === "hyperplan") {
		...
		const fallbackModelId =
			modelIdForTurn(params, task, fallbackHarnessId) ??
			getDefaultModelForHarness(fallbackHarnessId);
		const strategy =
	...
	const prompt = buildOpenADEPrompt({
		...
		input: params.input,
		comments: task.comments as Parameters<
			typeof buildOpenADEPrompt
		>[0]["comments"],
		label: params.label,
		includeComments: params.includeComments,
		planEventId,
	});
	const previousSnapshotPatch = prompt.createSnapshot
		? await latestSnapshotPatch(task)
		: undefined;
	const mcpServerConfigs = await buildRuntimeMcpServerConfigs(
		yjsStorage,
		task.enabledMcpServerIds,
	);
	...
	const activeExecution: ActiveTaskExecution = {
		executionId,
		runtimeId,
		repoId: params.repoId,
		eventId: createdEvent.eventId,
...
async function startReviewTurn({
	...
	const task = await readTaskForMutation(
		projection,
		params.repoId,
		params.taskId,
	);
	if (!canExecuteTaskInRuntime(task))
		throw new Error(
			"Server-owned review supports head and worktree tasks only",
		);
	const repo = (await projection.readProjects()).find(
		(project) => project.id === params.repoId,
	);
	...
	const latestPlanExecution =
		typeof latestPlan?.execution === "object" &&
		latestPlan.execution !== null &&
		!Array.isArray(latestPlan.execution)
			? (latestPlan.execution as Record<string, unknown>)
			: undefined;
	const latestPlanEvents = Array.isArray(latestPlanExecution?.events)
		? latestPlanExecution.events.filter(
				(event): event is Record<string, unknown> =>
					typeof event === "object" && event !== null && !Array.isArray(event),
			)
		: [];
	const latestPlanHarnessId =
		typeof latestPlanExecution?.harnessId === "string"
			? latestPlanExecution.harnessId
			: params.harnessId;
	const planText = latestPlan
		? (extractOpenADEPlanText(latestPlanEvents, latestPlanHarnessId) ?? "")
		: "";
	const reviewPrompt =
	...
	const createdEvent = await writer.createActionEvent({
		...
		source: {
			...
			userInstructions: reviewPrompt.userMessage,
		},
		includesCommentIds: [],
		modelId: params.modelId,
		gitRefsBefore,
	...
	void runHeadModeTurnExecution({
		...
		snapshotBase: executionEnvironment.snapshotBase,
		mcpServerConfigs: await buildRuntimeMcpServerConfigs(
			yjsStorage,
			task.enabledMcpServerIds,
		),
		runtimeId,
		...
		onCompleted: async ({ events }) => {
			...
			const currentTask = await readTaskForMutation(
				projection,
				params.repoId,
				params.taskId,
			);
			const followUpLabel = `${userLabel} Follow-up`;
			const handoffMessage = buildOpenADEReviewHandoffPrompt({
				reviewType: params.reviewType,
				reviewText,
			});
			...
			const followUpGitRefsBefore = await getGitRefs(
				followUpEnvironment.rootPath,
			);
			...
			const followUpActiveExecution: ActiveTaskExecution = {
				executionId: followUpExecutionId,
				runtimeId,
				repoId: params.repoId,
				eventId: followUpEvent.eventId,
			...
			void runHeadModeTurnExecution({
				...
				mcpServerConfigs: await buildRuntimeMcpServerConfigs(
					yjsStorage,
					currentTask.enabledMcpServerIds,
				),
...
async function runHeadModeTurnExecution({
	...
	const enqueue = (write: Promise<unknown>) => {
		pendingWrites.push(
			write.catch((error) =>
				console.warn("[RuntimeGateway] Failed to persist stream event:", error),
			),
		);
	};
	let finalized = false;
	const finalize = async (
		...
		if (terminalStatus === "completed") {
			...
			if (gitRefsAfter)
				await writer.updateActionExecution({ taskId, eventId, gitRefsAfter });
			...
			const failed = server.supervisor.update(runtimeId, {
				status: "failed",
				error,
			});
	...
	try {
		const start = await startRuntimeHarnessQuery({
			...
			onEvent(event) {
				...
				enqueue(
					writer.appendActionStreamEvent({
						taskId,
						eventId,
						streamEvent: event as Record<string, unknown> & { id: string },
					}),
				);
				if (
					event.direction === "execution" &&
					event.type === "session_started"
				) {
					savedSessionId = event.sessionId;
					enqueue(
						writer.updateActionExecution({
							taskId,
							eventId,
...
async function startHyperPlanTurn({
	...
	if (errors.length > 0)
		throw new Error(`Invalid HyperPlan strategy: ${errors.join(", ")}`);
	if (!canExecuteTaskInRuntime(task))
		throw new Error(
			"Server-owned HyperPlan supports head and worktree tasks only",
		);
	...
	const terminalStep = strategy.steps.find(
		(step) => step.id === strategy.terminalStepId,
	);
	if (!terminalStep)
		throw new Error(
			`Terminal HyperPlan step ${strategy.terminalStepId} not found`,
		);
	const mcpServerConfigs = await buildRuntimeMcpServerConfigs(
		yjsStorage,
		task.enabledMcpServerIds,
	...
	server.notify("runtime/updated", runtime);
	activeTaskExecutions.set(task.id, {
		executionId,
		runtimeId,
		repoId: params.repoId,
		eventId: createdEvent.eventId,
...
async function runHyperPlanTurnExecution({
	...
	try {
		...
		await finalize(
			"failed",
			error instanceof Error ? error.message : "HyperPlan failed",
		);
	}
}
...
async function runHyperPlanStep({
	...
	if (step.primitive === "plan") {
		...
		prompt = buildOpenADEReviewStepPrompt(
			taskDescription,
			inputText,
			inputStepId,
		);
		...
		const inputs = step.inputs
			.map((inputId) => {
				...
				const inputStep = strategy.steps.find(
		...
		await writer.setHyperPlanReconcileLabels({
			taskId,
			eventId,
			mapping: reconciled.labelMapping,
		});
		prompt = reconciled;
	} else {
	...
	const persist = (write: Promise<unknown>) => {
		persistedWrites.push(
			write.catch((error) =>
				console.warn(
					"[RuntimeGateway] Failed to persist HyperPlan event:",
					error,
				),
			),
		);
	...
	let settledResult:
		| Parameters<
				NonNullable<Parameters<typeof startRuntimeHarnessQuery>[0]["onSettled"]>
		  >[0]
		| undefined;
	const harnessPrompt =
		step.primitive === "plan"
			? await buildHarnessPrompt(prompt.userMessage, images)
			: prompt.userMessage;
	...
	const settled = new Promise<HyperPlanStepResult>((resolve) => {
		void startRuntimeHarnessQuery({
			...
			options: {
				...
				cwd,
				model: getModelFullId(
					step.agent.modelId,
					step.agent.harnessId as HarnessId,
				),
				mode: "read-only",
			...
			onEvent(event) {
				...
				if (isTerminal) {
					persist(
						writer.appendActionStreamEvent({ taskId, eventId, streamEvent }),
					);
				} else {
					persist(
						writer.appendHyperPlanSubExecutionStreamEvent({
							taskId,
							eventId,
							stepId: step.id,
							streamEvent,
			...
			.then((start) => {
				if (!start.ok && !settledResult) {
					resolve({
						status: "error",
						error: start.error ?? "Failed to start HyperPlan step",
					});
				}
			})
			.catch((error) => {
				resolve({
					...
					error:
						error instanceof Error
							? error.message
							: "Failed to start HyperPlan step",
				});
			});
	});
	...
	if (!runtimeServer) {
		runtimeServer = new RuntimeServer({
			...
			runHandlerWithContext: (event, run) =>
				runWithYjsDocumentOperationContext(
					{ runtimeMethod: event.method, runtimeRequestId: event.requestId },
					() =>
						runWithDataFolderOperationContext(
							{ runtimeMethod: event.method, runtimeRequestId: event.requestId },
							run,
