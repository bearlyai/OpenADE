...
interface TaskMcpSelectorProps {
    ...
    iconOnly?: boolean
    /** Vertical layout - stacks items in a column */
    vertical?: boolean
}
    ...
    ({ selectedServerIds, onSelectionChange, compact = false, iconOnly = false, vertical = false }: TaskMcpSelectorProps) => {
        ...
        const layoutClass = vertical
            ...
            : compact
              ...
              : "flex items-center gap-2 flex-wrap pt-4 mt-4 border-t border-border"

        return (
            <div className={layoutClass}>
                {!iconOnly && !vertical && <span className="text-xs text-muted shrink-0">Connectors:</span>}
                ...
                <button
                    type="button"
                    onClick={handleManage}
                    className="btn btn-sm h-auto px-2 py-1.5 btn-ghost text-muted hover:text-base-content flex items-center gap-1.5"
                >
                    <Settings size={12} />
