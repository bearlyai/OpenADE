...
export type MobileChromeStatusTone = "ok" | "warn" | "bad" | "muted"

export interface MobileChromeStatus {
    label: string
    tone: MobileChromeStatusTone
}

export interface MobileChromeNavItem<TScreen extends string> {
    screen: TScreen
    label: string
    icon: LucideIcon
}

export function mobileStatusToneClass(tone: MobileChromeStatusTone): string {
...
export function MobileChrome<TScreen extends string>({
    className,
    title,
    host,
    status,
    ...
    activeNav: TScreen
    navItems: Array<MobileChromeNavItem<TScreen>>
    children?: ReactNode
    onBack: () => void
    onRefresh: () => void
    ...
    return (
        ...
        >
            <MobileHeader title={title} host={host} status={status} showBack={showBack} isLoading={isLoading} onBack={onBack} onRefresh={onRefresh} />
            ...
            <MobileBottomNav active={activeNav} items={navItems} onNavigate={onNavigate} />
        </main>
    )
}
                ...
                <div className="flex min-w-0 flex-1 items-center gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="flex min-w-0 items-center gap-1 text-[11px] text-muted">
                            ...
                            <CircleDot size={9} className={mobileStatusToneClass(status.tone)} />
                            <span className={`shrink-0 ${mobileStatusToneClass(status.tone)}`}>{status.label}</span>
                        </div>
                    </div>
                </div>
...
function MobileBottomNav<TScreen extends string>({
    active,
    items,
    onNavigate,
}: {
    active: TScreen
    items: Array<MobileChromeNavItem<TScreen>>
    onNavigate: (screen: TScreen) => void
}) {
    return (
        <nav
            className="grid h-14 w-full max-w-full shrink-0 overflow-hidden border-t border-border bg-base-100"
