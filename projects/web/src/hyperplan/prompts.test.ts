...
describe("buildReviewStepPrompt", () => {
    ...
    it("system prompt includes review dimensions", () => {
        const result = buildReviewStepPrompt("task", "plan", "plan_0")
        expect(result.systemPrompt).toContain("<review_dimensions>")
        expect(result.systemPrompt).toContain("**Security**")
        expect(result.systemPrompt).toContain("**Test quality**")
        expect(result.systemPrompt).toContain("**Simplicity**")
        expect(result.systemPrompt).toContain("**Robustness**")
    })
})
...
describe("buildReconcileStepPrompt", () => {
    ...
    it("evaluation rubric includes security and robustness dimensions", () => {
        const result = buildReconcileStepPrompt("task", [planA, planB])
        expect(result.systemPrompt).toContain("Security")
        expect(result.systemPrompt).toContain("Robustness")
    })
    ...
    it("labelMapping labels match the XML ids", () => {
        ...
        for (const { label } of result.labelMapping) {
            expect(result.userMessage).toContain(`id="${label}"`)
        }
    })

    it("labelMapping includes reviews and plans", () => {
        const review: ReconcileInput = {
            stepId: "review_0",
            primitive: "review",
            text: "Review content",
        ...
        const stepIds = result.labelMapping.map((m) => m.stepId).sort()
        expect(stepIds).toEqual(["plan_0", "review_0"])
    })
