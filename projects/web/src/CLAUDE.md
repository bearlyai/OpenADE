...
Low-level harness IPC event/query types come from `@openade/harness/browser` as `HarnessIpc*`; `electronAPI/harnessEventTypes.ts` may keep renderer helper functions and compatibility aliases, but it must not redeclare the Electron harness bridge contract.
...
Classic desktop task event streams must keep task opening responsive. `components/InlineMessages` should render long histories tail-first and lazy-mount collapsed row/pill content; do not eagerly mount all historical tool output, diffs, stderr, or markdown just because the latest task event auto-expands.
