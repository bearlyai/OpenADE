...
function queuedTurnIdForClientRequest(
	...
	if (!clientRequestId?.trim()) return undefined;
	return `queued-${createHash("sha256").update(`${taskId}\0${clientRequestId}`).digest("hex").slice(0, 26)}`;
}
...
function cronInstallStateFromUnknown(
	key: string,
	value: unknown,
...
async function readNodeCronInstallState(
	cronDir: string,
	...
	try {
		...
		return {
			...
			installations: parseCronInstallStateDocument(raw).installations,
		};
	} catch (error) {
		if (nodeErrorCode(error) === "ENOENT")
			return { repoId: params.repoId, installations: {} };
		throw error;
	}
}
...
async function replaceNodeCronInstallState(
	...
	const document: CronInstallStateDocument = {
...
function isScopedTaskBinaryPath(filePath: string): boolean {
	return SCOPED_TASK_BINARY_EXTENSIONS.has(
...
async function scopedGitRequire(
	args: string[],
	cwd: string,
	label: string,
): Promise<string> {
...
function parseScopedNameStatusOutput(
	stdout: string,
): OpenADETaskGitChangedFile[] {
...
function parseScopedNumstatStats(stdout: string): OpenADETaskGitChangeStats {
	...
	for (const line of stdout.trim().split("\n").filter(Boolean)) {
		...
		if (additions !== "-")
			insertions += Number.parseInt(additions ?? "0", 10) || 0;
		if (removals !== "-")
			deletions += Number.parseInt(removals ?? "0", 10) || 0;
	}
...
function scopedTaskSummaryFiles(stdout: string): OpenADETaskGitChangedFile[] {
	return parseScopedNameStatusOutput(stdout).map((file) => ({
		...file,
		binary: isScopedTaskBinaryPath(file.path),
	}));
}
...
async function scopedGitSummary(workDir: string): Promise<ScopedGitSummary> {
	...
	if (branch) {
		const upstreamAhead = await scopedGit(
	...
	const stagedFiles = stagedNameStatus.success
		...
		: [];
	const unstagedFiles = unstagedNameStatus.success
		? scopedTaskSummaryFiles(unstagedNameStatus.stdout)
		: [];
	const untracked = untrackedResult.success
		? untrackedResult.stdout
				...
				.map((filePath) => ({
					...
					binary: isScopedTaskBinaryPath(filePath),
				}))
		: [];
	...
	return {
		...
		staged: {
			files: stagedFiles,
			stats: stagedNumstat.success
				? parseScopedNumstatStats(stagedNumstat.stdout)
				: { filesChanged: 0, insertions: 0, deletions: 0 },
		},
		unstaged: {
			files: unstagedFiles,
			stats: unstagedNumstat.success
				? parseScopedNumstatStats(unstagedNumstat.stdout)
				: { filesChanged: 0, insertions: 0, deletions: 0 },
		},
		untracked,
	};
...
function scopedTaskGitBranchScopes(
	stdout: string,
...
function scopedTaskGitDefaultBranch(workDir: string): Promise<string> {
	return (async () => {
		const remoteHeadResult = await scopedGit(
		...
		const mainResult = await scopedGit(
			["show-ref", "--verify", "refs/heads/main"],
			workDir,
		);
		if (mainResult.success) return "main";

		const masterResult = await scopedGit(
...
async function scopedProjectGitInfo(
...
function scopedProjectGitBranchesFromOutput(
	stdout: string,
	defaultBranch: string,
	isRemote: boolean,
): OpenADEProjectGitBranchesReadResult["branches"] {
	return stdout
		.split("\n")
		.map((branch) => normalizeScopedGitBranchName(branch))
		.filter((branch) => branch.length > 0 && !branch.includes("HEAD"))
...
async function scopedProjectGitBranches(
	...
	const localBranchesResult = await scopedGitRequire(
		...
		gitInfo.repoRoot,
		"Failed to list project git branches",
	);
	const branches = scopedProjectGitBranchesFromOutput(
		localBranchesResult,
	...
	if (params.includeRemote) {
		...
		if (remoteBranchesResult.success) {
			for (const remoteBranch of scopedProjectGitBranchesFromOutput(
...
async function scopedProjectGitSummary(
...
function scopedTaskGitWorktreeEntries(
	stdout: string,
): Array<{ scope: OpenADETaskGitWorktreeScope; path: string }> {
	...
	const pushCurrent = () => {
		...
		const branch = current.branch
			? normalizeScopedGitBranchName(current.branch)
			: "";
		entries.push({
			path: current.path,
			scope: {
...
function scopedTaskGitWorktreeScopes(
	stdout: string,
): OpenADETaskGitWorktreeScope[] {
...
async function scopedTaskGitWorkDirForLog(
	...
	return (
		scopedTaskGitWorktreeEntries(worktreeScopesResult.stdout).find(
			(entry) => entry.scope.worktreeId === worktreeId,
		)?.path ?? defaultWorkDir
	);
}
...
async function scopedTaskGitScopes(
	...
	if (remoteBranchesResult) {
		if (remoteBranchesResult.success) {
			const localBranchNames = new Set(
				branchScopes
					.filter(
						(scope): scope is OpenADETaskGitBranchScope =>
							scope.type === "branch",
					)
					.map((scope) => scope.name),
			);
			for (const remoteBranch of scopedTaskGitBranchScopes(
				remoteBranchesResult.stdout,
				defaultBranch,
				true,
			)) {
				const localName = remoteBranch.name.replace(/^origin\//, "");
				if (
					!localBranchNames.has(localName) &&
					!localBranchNames.has(remoteBranch.name)
				) {
	...
	return {
		...
		defaultBranch,
		scopes: headScope
			? [headScope, ...sortableBranches, ...worktreeScopes]
			: [...sortableBranches, ...worktreeScopes],
	};
}
	...
	if (params.task.isolationStrategy?.type === "worktree") {
		const result = await scopedGit(
			[
				"merge-base",
				"--is-ancestor",
				`openade/${params.task.slug}`,
				params.task.isolationStrategy.sourceBranch,
			],
			params.repo.path,
		).catch(() => null);
...
async function generateNodeTaskTitle(
	...
	const harnessId =
		params.harnessId ??
		actionHarnessId(
			latestCompletedPlanEvent(params.task.events),
			"claude-code",
		);
	...
	const settled = await new Promise<{ ok: boolean; error?: string }>(
		(resolve) => {
			let done = false;
...
function latestTaskMergeBase(task: OpenADETask): string | undefined {
	for (let index = task.deviceEnvironments.length - 1; index >= 0; index--) {
		...
		if (environment.setupComplete && environment.mergeBaseCommit)
			return environment.mergeBaseCommit;
	}
	for (let index = task.events.length - 1; index >= 0; index--) {
		const event = eventRecord(task.events[index]);
		if (
			event?.type === "snapshot" &&
			typeof event.mergeBaseCommit === "string" &&
			event.mergeBaseCommit.length > 0
		) {
...
function validateScopedTaskEnvironmentSlug(slug: string): string {
	if (!/^[A-Za-z0-9._-]+$/.test(slug))
		throw new Error("Task slug is invalid for environment setup");
	return slug;
}

async function scopedGitRepositoryInfo(
	directory: string,
): Promise<{ repoRoot: string; relativePath: string; defaultBranch: string }> {
	const repoRoot = (
...
async function scopedTaskEnvironmentMergeBase(params: {
	repoRoot: string;
	worktreeBranch: string;
	sourceBranch: string;
}): Promise<string | undefined> {
	const result = await scopedGit(
		["merge-base", params.worktreeBranch, params.sourceBranch],
		params.repoRoot,
	);
...
async function scopedTaskEnvironmentWorktree(params: {
	...
	const worktrees = await scopedGit(
		["worktree", "list", "--porcelain"],
		params.repoRoot,
	);
	if (worktrees.success) {
		const existing = scopedTaskGitWorktreeEntries(worktrees.stdout).find(
			(entry) => entry.path === worktreeDir,
		);
		if (
			existing?.scope.branch === worktreeBranch ||
			existing?.scope.branch === expectedBranch
		) {
			return {
				worktreeDir,
				mergeBaseCommit: await scopedTaskEnvironmentMergeBase({
		...
		if (existing) {
			await scopedGitRequire(
				...
				params.repoRoot,
				"Failed to remove mismatched task worktree",
			);
		}
	}
	...
	if (!added.success && added.stderr.includes("already exists")) {
		added = await scopedGit(
			["worktree", "add", worktreeDir, worktreeBranch],
			params.repoRoot,
		);
	}
	if (!added.success)
		throw new Error(
			`Failed to create task worktree: ${added.stderr || added.stdout}`,
		);
...
async function prepareScopedTaskEnvironment(
	...
	const gitInfo = await scopedGitRepositoryInfo(params.repo.path);
	const sourceBranch =
		isolationStrategy.sourceBranch || gitInfo.defaultBranch || "main";
	const worktree = await scopedTaskEnvironmentWorktree({
		repoRoot: gitInfo.repoRoot,
		slug: params.task.slug,
		sourceBranch,
	});
	const workingDir = gitInfo.relativePath
		? path.join(worktree.worktreeDir, gitInfo.relativePath)
...
function scopedTaskFromTreeish(
	...
	if (isolationStrategy.type === "worktree")
		return latestTaskMergeBase(task) ?? "HEAD";
	return "HEAD";
}
...
async function scopedTaskChanges(
	...
	const toTreeish = "";
	const output = await scopedGitRequire(
		["diff", "--name-status", "-M", fromTreeish],
		workDir,
	...
	const untracked = await scopedGit(
...
function parseScopedPatchStats(
	patch: string,
): Pick<OpenADETaskDiffStats, "insertions" | "deletions"> {
	let insertions = 0;
	let deletions = 0;
...
function finalizeScopedTaskDiffPatch(
	...
	const truncated =
		allowTruncation && patch.length > MAX_SCOPED_TASK_PATCH_SIZE;
...
function resolveTaskGitFilePath(
	workDir: string,
	filePath: string,
...
async function scopedUntrackedTaskFilePatch(params: {
	...
	if (!result.success && result.code !== 1) {
		throw new Error(
			`Failed to get untracked task file patch: ${result.stderr}`,
		);
	}
	return finalizeScopedTaskDiffPatch(
		result.stdout,
...
async function scopedTaskDiff(
	...
	if (
		SCOPED_TASK_GENERATED_FILE_BASENAMES.has(path.basename(params.filePath))
	) {
		return createEmptyTaskDiffReadResult(resultBase);
	}

	const pathspecs = taskGitDiffPathspecs(
		workDir,
		params.filePath,
		params.oldPath,
	);
	const result = await scopedGit(
		["diff", "-M", `-U${contextLines}`, fromTreeish, "--", ...pathspecs],
		workDir,
	);
	if (!result.success)
		throw new Error(`Failed to get task file patch: ${result.stderr}`);

	const response = finalizeScopedTaskDiffPatch(
		result.stdout,
		allowTruncation,
		resultBase,
	);
...
async function readScopedTaskWorkingTreeFile(
	workDir: string,
	filePath: string,
...
async function scopedTaskFilePair(
	...
	const before = await readScopedTaskFileAtTreeish(
		workDir,
		fromTreeish,
		beforePath,
	);
...
async function scopedTaskGitLog(
	params: OpenADETaskGitLogRequest & {
		repo: OpenADEProject;
		task: OpenADETask;
	},
	...
	const limit = Math.max(
		1,
		Math.min(
			params.limit ?? DEFAULT_SCOPED_TASK_GIT_LOG_LIMIT,
			MAX_SCOPED_TASK_GIT_LOG_LIMIT,
		),
	);
	...
	const format =
		["%H", "%h", "%s", "%an", "%aI", "%ar", "%P"].join(fieldSeparator) +
		recordSeparator;
	const args = [
		"log",
		`--format=${format}`,
...
async function scopedTaskGitCommitFiles(
	...
	const workDir = await scopedTaskWorkDir(params.repo, params.task);
	const parentOutput = await scopedGitRequire(
		["show", "--no-patch", "--format=%P", params.commit],
		workDir,
		"Failed to resolve commit parents",
	);
	...
	const diffOutput =
		parents.length === 0
			...
			: await scopedGitRequire(
					["diff", "--name-status", "-M", parents[0], params.commit],
					workDir,
					"Failed to get commit files",
				);
...
async function scopedTaskGitFileAtTreeish(
	...
	const workDir = await scopedTaskWorkDir(params.repo, params.task);
	const result = await readScopedTaskFileAtTreeish(
		workDir,
		params.treeish,
		params.filePath,
	);
...
async function scopedTaskGitCommitFilePatch(
	...
	if (!result.success)
		throw new Error(
			`Failed to get commit file patch: ${result.stderr || result.stdout}`,
		);
	return finalizeScopedTaskGitCommitFilePatch(
		result.stdout,
		allowTruncation,
		resultBase,
	);
...
async function scopedTaskGitCommit(
	...
	if (!statusResult.success) {
		return {
			...
			error:
				statusResult.stderr ||
				statusResult.stdout ||
				"Failed to inspect task changes",
		};
	}
	...
	const commitResult = await scopedGit(
...
async function loadNodeSnapshotPatch(
	snapshotDir: string,
	patchFileId: string,
): Promise<string | null> {
	return fs
		.readFile(path.join(snapshotDir, `${patchFileId}.patch`), "utf8")
		.catch(() => null);
}

async function loadNodeSnapshotIndex(
	snapshotDir: string,
	patchFileId: string,
): Promise<OpenADESnapshotPatchIndex | null> {
...
function nodeDataFilePath(baseDir: string, id: string, ext: string): string {
	...
	if (filePath !== path.join(root, `${id}.${ext}`))
...
async function readNodeTaskImage(
	...
	const filePath = nodeDataFilePath(
		params.imageDir,
		params.imageId,
		params.ext,
	);
...
function nodeTaskImageMediaType(
	ext: string,
): OpenADETaskImageStagedReadResult["mediaType"] {
	switch (ext) {
		case "gif":
			return "image/gif";
		case "jpeg":
...
async function readNodeStagedTaskImage(
	params: OpenADETaskImageStagedReadRequest & { imageDir: string },
...
async function writeNodeTaskImage(
	params: OpenADETaskImageWriteRequest & { imageDir: string },
...
const NODE_PROCS_SKIP_DIRS = new Set([
	".git",
	"node_modules",
	"dist",
	"build",
...
async function nodeProjectProcessGitInfo(searchRoot: string): Promise<{
	...
	const repoRoot = await scopedGit(
		["rev-parse", "--show-toplevel"],
		searchRoot,
	);
...
async function walkNodeOpenADEProcsFiles(
	root: string,
	files: string[] = [],
	depth = 0,
): Promise<string[]> {
	...
	for (const entry of entries) {
		if (entry.isFile() && entry.name === NODE_PROCS_CONFIG_FILENAME) {
			...
			entry.isDirectory() &&
			!NODE_PROCS_SKIP_DIRS.has(entry.name) &&
			!entry.name.startsWith(".")
		) {
			await walkNodeOpenADEProcsFiles(
...
async function findNodeOpenADEProcsFiles(
	searchRoot: string,
	repoRoot: string,
	hasGitInfo: boolean,
): Promise<string[]> {
...
async function readNodeProjectProcsConfigs(
	...
	const configFiles = await findNodeOpenADEProcsFiles(
		resolvedSearchRoot,
		repoRoot,
		gitInfo !== null,
	);
...
async function nodeRuntimeRequest(
	...
	if (!server)
		throw new RuntimeHandlerError(
			"host_unavailable",
			"Scoped project process host requires a runtime server",
		);
	const response = await server.handleRequest(
		{
			id: `openade-process-${++scopedProjectProcessRequestId}`,
			method,
			params,
		},
		{
	...
	if (response.error) {
		const message =
			response.error.code === "method_not_found"
				? "Scoped project process host requires the runtime process module"
...
async function listNodeProjectProcesses(
	params: OpenADEProjectProcessListRequest & {
		repo: OpenADEProject;
		task?: OpenADETask;
		registry: Map<string, OpenADEProjectProcessRegistration>;
		server?: RuntimeServer;
	},
): Promise<OpenADEProjectProcessListResult> {
	const searchRoot = await scopedProjectProcessSearchRoot(params);
	const result = await readNodeProjectProcessDefinitions(searchRoot);
	const rawList = eventRecord(
		await nodeRuntimeRequest(params.server, "process/list").catch(() => ({
			processes: [],
		})),
	);
	const rawProcesses = Array.isArray(rawList?.processes)
		? rawList.processes
		: [];
	...
	for (const rawProcess of rawProcesses) {
		...
		if (
			...
			continue;
		const instance = openADEProjectProcessInstanceFromUnknown(
			rawProcess,
			registration,
		);
...
async function startNodeProjectProcess(
	...
	const searchRoot = await scopedProjectProcessSearchRoot(params);
	const result = await readNodeProjectProcessDefinitions(searchRoot);
	const processDef = result.processes.find(
...
async function reconnectNodeProjectProcess(
	...
	return openADEProjectProcessReconnectResultFromUnknown(
		await nodeRuntimeRequest(params.server, "process/reconnect", {
			processId: params.processId,
		}),
		params,
	);
...
async function stopNodeProjectProcess(
	...
	const result = openADEProjectProcessStopResultFromUnknown(
		await nodeRuntimeRequest(params.server, "process/kill", {
			processId: params.processId,
		}),
		params,
	);
...
function taskTerminalMutationResult(
	value: unknown,
	params: { repoId: string; taskId: string; terminalId: string },
): OpenADETaskTerminalMutationResult {
...
async function writeNodeTaskTerminal(
	...
	const result = await nodeRuntimeRequest(params.server, "pty/write", {
...
async function resizeNodeTaskTerminal(
	...
	assertOpenADETaskTerminalId(params);
	const result = await nodeRuntimeRequest(params.server, "pty/resize", {
		ptyId: params.terminalId,
		cols: params.cols,
		rows: params.rows,
	});
...
async function stopNodeTaskTerminal(
	...
	assertOpenADETaskTerminalId(params);
	const result = await nodeRuntimeRequest(params.server, "pty/kill", {
		ptyId: params.terminalId,
	});
	return taskTerminalMutationResult(result, params);
}
...
function actionHarnessId(
	...
	const execution = eventRecord(event?.execution);
	return typeof execution?.harnessId === "string"
		? execution.harnessId
		: fallback;
}
...
function taskReviewThreadXml(task: OpenADETask): string {
	const events = task.events.filter(
		(event) => eventRecord(event)?.type !== "snapshot",
	);
...
async function reconcileCheckpointedOpenADEActionEvents({
	...
	for (const runtime of server.supervisor.list({
		...
		if (result?.changed && result.repoId)
			notifyTaskChanged(server, result.repoId, taskId);
	}
}
...
async function stopActiveOpenADERuntime({
	...
	const executionIds =
		active.childExecutionIds && active.childExecutionIds.size > 0
	...
	if (failed) {
		throw new RuntimeHandlerError(
			"stop_failed",
			failed.error ?? "Failed to stop OpenADE runtime",
...
export function createRuntimeNodeOpenADEAdapters(
	options: RuntimeNodeOpenADEOptions = {},
): OpenADEModuleAdapters {
	...
	const agentExecutor =
		options.agentExecutor ?? createRuntimeNodeHarnessAgentExecutor();
	...
	async function startTurn(
	...
	): Promise<OpenADETurnStartResult> {
		...
		const project = (await projection.readProjects()).find(
			(candidate) => candidate.id === params.repoId,
		);
		...
		if (params.type === "hyperplan") {
			...
			if (!strategy) {
				...
				return attachAcceptedTaskStart({
					repoId: params.repoId,
					taskId,
					result,
					includeTask: createdTask,
				});
			}
			...
			return startHyperPlan({
				params,
				task,
				project,
				strategy,
				context,
				includeAcceptedTask: createdTask,
			});
		}
		...
		activeTaskExecutions.set(taskId, {
			executionId,
			runtimeId,
			repoId: params.repoId,
			eventId: action.eventId,
		...
		void runHeadlessTurn({
			...
			appendSystemPrompt:
				[prompt.systemPrompt, params.appendSystemPrompt]
					.filter(Boolean)
					.join("\n\n") || undefined,
	...
	async function attachAcceptedTaskStart({
		repoId,
		taskId,
		result,
		includeTask,
		...
		if (!includeTask) return result;
		const acceptedTask = await projection.readTask(repoId, taskId);
		const previews = await projection.readTaskList(repoId);
		return {
			...result,
			task: acceptedTask,
			preview: previews.find(
				(candidate: OpenADETaskPreview) => candidate.id === taskId,
			),
		};
	}
	...
	async function startReview(
		...
		activeTaskExecutions.set(params.taskId, {
			executionId,
			runtimeId,
			repoId: params.repoId,
			eventId: action.eventId,
		...
		void runHeadlessTurn({
			...
			onCompleted: async ({ events }) => {
				...
				const followUpMessage = buildOpenADEReviewHandoffPrompt({
					reviewType: params.reviewType,
					reviewText,
				});
	...
	async function startHyperPlan({
		...
		const action = await writer.createActionEvent({
			...
			source: {
				type: "hyperplan",
				userLabel: params.label ?? "HyperPlan",
		...
		return attachAcceptedTaskStart({
			...
			includeTask: includeAcceptedTask,
		});
	}

	const snapshotPatchStore = {
		loadPatch: (patchFileId: string) =>
			loadNodeSnapshotPatch(snapshotDir, patchFileId),
		loadIndex: (patchFileId: string) =>
			loadNodeSnapshotIndex(snapshotDir, patchFileId),
	};
	...
	return {
		...
		scopedHost: {
			...
			startTaskTerminal: (params) =>
				startNodeTaskTerminal({ ...params, server }),
			reconnectTaskTerminal: (params) =>
				reconnectNodeTaskTerminal({ ...params, server }),
			writeTaskTerminal: (params) =>
		...
		readStagedTaskImage: (params) =>
			readNodeStagedTaskImage({ ...params, imageDir }),
		saveDataDocumentBase64: (id, data) =>
			storage.saveDocumentUpdate(id, Buffer.from(data, "base64")),
		...
		interruptTurn: async (params) => {
			...
			if (!active)
				return {
					ok: false,
					error: "No active headless turn is running for this task",
				};
			active.stopping = true;
			const executionIds =
				active.childExecutionIds && active.childExecutionIds.size > 0
		...
		reorderQueuedTurns: async (params) => {
			...
			const remainingTurns = (task.queuedTurns ?? []).filter(
				(turn) => !requestedIds.has(turn.id),
			);
			const nextTurns = [...requestedTurns, ...remainingTurns];
		...
		reconcileActionEventRuntime: async (params) => {
			...
			if (result.changed && result.repoId)
				notifyTaskChanged(server, result.repoId, params.taskId);
			return result;
		},
		updateActionExecution: (params) => writer.updateActionExecution(params),
		addHyperPlanSubExecution: (params) =>
			writer.addHyperPlanSubExecution(params),
		appendHyperPlanSubExecutionStreamEvent: (params) =>
			writer.appendHyperPlanSubExecutionStreamEvent(params),
		updateHyperPlanSubExecution: (params) =>
			writer.updateHyperPlanSubExecution(params),
		setHyperPlanReconcileLabels: (params) =>
			writer.setHyperPlanReconcileLabels(params),
...
export function registerRuntimeNodeOpenADEModule(
	server: RuntimeServer,
	options: RuntimeNodeOpenADEOptions = {},
): void {
	const agentExecutor =
		options.agentExecutor ?? createRuntimeNodeHarnessAgentExecutor();
	if (options.registerAgentModule !== false)
...
async function runHeadlessHyperPlanStep({
	...
	if (step.primitive === "plan") {
		prompt = buildOpenADEHyperPlanStepPrompt(taskDescription, {
			mainThreadContextXml,
		});
	...
	const persist = (write: Promise<unknown>) => {
		persistedWrites.push(
			write.catch((error) =>
				console.warn(
					"[RuntimeNodeOpenADE] Failed to persist HyperPlan stream event:",
					error,
				),
			),
		);
...
async function runHeadlessTurn({
	...
	const enqueue = (write: Promise<unknown>) => {
		pendingWrites.push(
			write.catch((error) =>
				console.warn(
					"[RuntimeNodeOpenADE] Failed to persist stream event:",
					error,
				),
			),
		);
		...
		},
		{
			...
			onSettled(result) {
				if (result.status === "completed") void finalize("completed");
					...
					void finalize("stopped", result.error);
