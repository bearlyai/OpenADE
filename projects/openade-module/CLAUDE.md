...
- `OpenADEScopedHostAdapter` is the product-level host boundary for shared-shell host reads and tightly scoped writes. Add scoped methods here, such as `openade/project/files/tree`, `openade/project/file/read`, `openade/project/file/write`, `openade/project/search`, `openade/project/process/*`, `openade/task/terminal/*`, `openade/task/changes/read`, `openade/task/diff/read`, `openade/task/git/log`, `openade/task/git/commit`, `openade/task/image/read`, and `openade/task/snapshot/*` reads, instead of granting remote clients raw `fs/*`, `git/*`, `pty/*`, `process/*`, `snapshot/*`, `host/*`, or `data/*` methods. File writes must stay repo-relative and permissioned separately from read methods. Scoped process methods must resolve `openade.toml` process ids and cwd server-side. Scoped terminal methods must derive PTY ids server-side from repo/task ids. Scoped git commits must resolve task cwd server-side and remain ungranted to paired devices unless an explicit role/permission decision is made. Scoped image reads must verify the image is referenced by the task before loading bytes.
