...
- `src/scopedProjectProcesses.ts` owns scoped OpenADE project process definition building, cwd containment from parsed `openade.toml` configs, process registration/scope matching, timeout policy, and runtime process DTO normalization. Hosts may keep their own process lifecycle plumbing, but they should not duplicate config-to-definition, timeout, or DTO-shaping logic.
